...
import type { FontWeight, FontStyle, FontDecoration } from "./types.js";

export interface FontProperties extends Partial<Pick<Font, "decoration" | "family" | "style" | "weight">> {
  /**
   * The font size in points. This value may be autocast with a string
   ...
   * > **Known Limitations**
   * >
   * > - The maximum supported font size is 96 points or 128 pixels.
   * > - In 3D [SceneViews](https://developers.arcgis.com/javascript/latest/references/core/views/SceneView/), if the [TextSymbol3DLayer.size](https://developers.arcgis.com/javascript/latest/references/core/symbols/TextSymbol3DLayer/#size) property is set, it will override this [size](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#size) property.
   *
   * @default 9
   * @see [MDN: font-size](https://developer.mozilla.org/docs/Web/CSS/Reference/Properties/font-size)
   * @example
   * // size in points
   * symbol.size = 14;
   * @example
...
/**
 * The font used to display [2D text symbols](https://developers.arcgis.com/javascript/latest/references/core/symbols/TextSymbol/) and
 * [3D text symbols](https://developers.arcgis.com/javascript/latest/references/core/symbols/TextSymbol3DLayer/). This class allows the developer
 * to set the font's [family](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#family), [decoration](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#decoration), [size](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#size), [style](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#style), and [weight](https://developers.arcgis.com/javascript/latest/references/core/symbols/Font/#weight)
 * properties. Take note of the "Known Limitations" for each property to understand how they differ
 * depending on the layer type, and if you working with a [MapView](https://developers.arcgis.com/javascript/latest/references/core/views/MapView/)
 ...
 * @see [Deprecated since 5.1 - Guide: Esri Icon Font](https://developers.arcgis.com/javascript/latest/esri-icon-font/)
 * @see [Guide: Labeling](https://developers.arcgis.com/javascript/latest/labeling/)
 * @example
 * let textSymbol = {
 ...
 * };
 */
export default class Font extends JSONSupport {
   ...
   * @default "none"
   */
  accessor decoration: FontDecoration;
  /**
   * The font family of the text. The possible values are dependent upon the
   * layer type, and if you are working with a [MapView](https://developers.arcgis.com/javascript/latest/references/core/views/MapView/)
   * or a [SceneView](https://developers.arcgis.com/javascript/latest/references/core/views/SceneView/). See the [Labeling guide page](https://developers.arcgis.com/javascript/latest/labeling/)
   * for detailed explanation, or click the `Read more` below.
   *
   * <details>
   * <summary>Read More</summary>
   *
   * #### Font families for 3D SceneViews
   *
   * The supported font families for 3D [SceneViews](https://developers.arcgis.com/javascript/latest/references/core/views/SceneView/) are dependent upon the
   * fonts installed on the user's computer and web browser. If an app uses a font that is not installed, the
   * Font class implements a fallback mechanism that will use the default font family value, which is `sans-serif`.
   * See these references for instructions on how to install a new font on [Windows](https://support.microsoft.com/en-us/help/314960/how-to-install-or-remove-a-font-in-windows)
   * or [macOS](https://support.apple.com/en-us/ht201749).
   *
   * Fonts that are not installed locally can also be loaded from a url by defining a [@font-face](https://developer.mozilla.org/docs/Web/CSS/@font-face)
   * in a css file and referencing it from the `family` property in a symbol layer.
   *
   * #### Font families for 2D MapImageLayer
   *
   * The supported font families for [MapImageLayers](https://developers.arcgis.com/javascript/latest/references/core/layers/MapImageLayer/)
   * in a [MapView](https://developers.arcgis.com/javascript/latest/references/core/views/MapView/) are dependent upon the fonts installed on the
   * [ArcGIS Server](https://doc.esri.com/en/arcgis-enterprise/latest/plan/what-is-arcgis-server.html)
   * that published the layer. If an app uses a font that is not installed, the Font class implements a fallback
   * mechanism that will use the default font family value, which is `sans-serif`.
   *
   * #### Font families for 2D FeatureLayer, CSVLayer, StreamLayer, and TextSymbol
   *
   * The supported font families are based on hosted fonts files in `.pbf` format.
   * By default, the fonts available are mostly the same ones used by the Esri Vector Basemaps.
   * These fonts are available via `https://static.arcgis.com/fonts`. The URL can be configured to point to your own font resources
   * by setting the [esriConfig.fontsUrl](https://developers.arcgis.com/javascript/latest/references/core/config/#Config-fontsUrl) property. If an app uses a font that is not installed,
   * the Font class implements a fallback mechanism that will use the default font family value, which is `sans-serif`.
   * This uses the `Arial Unicode MS` font file.
   *
   * > A preview of the fonts listed in the following table are available in the [Labeling](https://developers.arcgis.com/javascript/latest/labeling/) overview page.
   *
   ...
   * | Avenir Next LT Pro Light              | normal | normal | font: { family: 'Avenir Next LT Pro Light', style: 'normal', weight: 'normal' } |
   ...
   * | BellTopo Sans Bold                    | normal | bold   | font: { family: 'BellTopo Sans', style: 'normal', weight: 'bold' }              |
   * | BellTopo Sans Bold Italic             | italic | bold   | font: { family: 'BellTopo Sans', style: 'italic', weight: 'bold' }              |
   * | BellTopo Sans Italic                  | italic | normal | font: { family: 'BellTopo Sans', style: 'italic', weight: 'normal' }            |
   * | BellTopo Sans Regular                 | normal | normal | font: { family: 'BellTopo Sans', style: 'normal', weight: 'normal' }            |
   ...
   * | FGDC GeoAge Regular                   | normal | normal | font: { family: 'FGDC GeoAge', style: 'normal', weight: 'normal' }              |
   ...
   * | Josefin Sans Semibold Italic          | italic | bold   | font: { family: 'Josefin Sans Semibold', style: 'italic', weight: 'bold' }      |
   ...
   * | Josefin Slab Semibold Italic          | italic | bold   | font: { family: 'Josefin Slab Semibold', style: 'italic', weight: 'bold' }      |
   ...
   * | Montserrat Semibold Italic            | italic | bold   | font: { family: 'Montserrat Semibold', style: 'italic', weight: 'bold' }        |
   ...
   * | Playfair Display Black                | normal | normal | font: { family: 'Playfair Display Black', style: 'normal', weight: 'normal' }   |
   ...
   * | Ubuntu Light                          | normal | normal | font: { family: 'Ubuntu Light', style: 'normal', weight: 'normal' }             |
   ...
   * @default "sans-serif"
   * @see [Guide: Labeling](https://developers.arcgis.com/javascript/latest/labeling/)
   * @see [MDN: font-family](https://developer.mozilla.org/docs/Web/CSS/Reference/Properties/font-family)
   * @example
   * let textSymbol = {
   *   type: "text",  // autocasts as new TextSymbol()
   ...
   * > **Known Limitations**
   * >
   * > `oblique` is not supported in 2D [FeatureLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/), [CSVLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/), and [StreamLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/StreamLayer/) labels.
   *
   * @default "normal"
   * @see [MDN: font-style](https://developer.mozilla.org/docs/Web/CSS/Reference/Properties/font-style)
   */
  accessor style: FontStyle;
  /**
   ...
   * > **Known Limitations**
   * >
   * > `bolder` and `lighter` are not supported in 2D [FeatureLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/), [CSVLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/), and [StreamLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/StreamLayer/) labels.
   *
   * @default "normal"
   * @see [MDN: font-weight](https://developer.mozilla.org/docs/Web/CSS/Reference/Properties/font-weight)
   */
  accessor weight: FontWeight;
  /**
