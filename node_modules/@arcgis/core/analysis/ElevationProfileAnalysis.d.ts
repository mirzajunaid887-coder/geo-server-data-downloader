...
import type ElevationProfileGroup from "./ElevationProfile/ElevationProfileGroup.js";
...
import type { ElevationProfileGroupProperties } from "./ElevationProfile/ElevationProfileGroup.js";
...
export interface ElevationProfileAnalysisProperties extends AnalysisProperties {
  ...
  /**
   * The collection of profiles to compute and display.
   *
   * A profile can be either:
   *
   * * An [ElevationProfileLine](https://developers.arcgis.com/javascript/latest/references/core/analysis/ElevationProfile/ElevationProfileLine/) that defines a source for sampling elevation
   * along the input geometry (such as ground, input geometry itself, custom elevation sources, or scene content),
   * and includes settings for how the resulting line will appear in charts and in the view.
   * * An [ElevationProfileGroup](https://developers.arcgis.com/javascript/latest/references/core/analysis/ElevationProfile/ElevationProfileGroup/) that organizes multiple profile lines under a single
   * logical container, represented as a single legend entry.
   *
   ...
   */
  profiles?: ReadonlyArrayOrCollection<((ElevationProfileLineGroundProperties & { type: "ground" }) | (ElevationProfileLineInputProperties & { type: "input" }) | (ElevationProfileLineQueryProperties & { type: "query" }) | (ElevationProfileLineSceneProperties & { type: "scene" })) | (ElevationProfileGroupProperties & { type: "group" })> | null;
 ...
 *
 * This analysis supports multiple profiles, each representing a different source or method for sampling elevation.
 * A profile can be a line (such as ground, input geometry, custom elevation sources, or scene content)
 * or a [group](https://developers.arcgis.com/javascript/latest/references/core/analysis/ElevationProfile/ElevationProfileGroup/). The configuration of these profiles is managed
 ...
 * The combined statistics, as well as results for each computed profile line, can then be visualized in a custom chart using any charting library,
...
export default class ElevationProfileAnalysis extends Analysis {
   ...
   * @see [Sample - Elevation Profile group](https://developers.arcgis.com/javascript/latest/sample-code/elevation-profile-group/)
   */
  get profiles(): Collection<ElevationProfileLineUnion | ElevationProfileGroup>;
  set profiles(value: ReadonlyArrayOrCollection<((ElevationProfileLineGroundProperties & { type: "ground" }) | (ElevationProfileLineInputProperties & { type: "input" }) | (ElevationProfileLineQueryProperties & { type: "query" }) | (ElevationProfileLineSceneProperties & { type: "scene" })) | (ElevationProfileGroupProperties & { type: "group" })> | null | undefined);
  /** The analysis type. */
