 ...
 * * [Notes](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#notes)
 *
 ...
 * longitude coordinate is the X value. The X, Y coordinates must be stored in [SpatialReference.WGS84](https://developers.arcgis.com/javascript/latest/references/core/geometry/SpatialReference/#WGS84) in csv feed.
 *
 * CSVLayer fetches all of its data once and store them on the client-side when it loads. CSVLayer, however, will make server-side requests
 * to fetch updated data if its [refreshInterval](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#refreshInterval) is set or [refresh()](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#refresh) method is called.
 * Refer to FeatureLayer's [querying your data](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/#querying) section to learn more about how to query your csv data, and
 * the [data visualization](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/#data-visualization) section to learn how to change the CSVLayer's visualization.
 *
 * <span id="creating-a-csvlayer"></span>
 * ## Creating a CSVLayer
 *
 * CSVLayer is created by setting its [url](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#url) property to point to csv feed or to a blob url for in memory csv data. Specify names of your latitude and longitude fields
 * by setting [latitudeField](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#latitudeField) and [longitudeField](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#longitudeField) properties if the csv data does not contain x, y coordinates in one of the predefined fields.
 * Values of latitude and longitude fields must be stored in WGS84 in csv feed. Hence CSVLayer's [spatialReference](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#spatialReference) will default to WGS84. However, you can
 * specify the layer's spatial reference to match the [View.spatialReference](https://developers.arcgis.com/javascript/latest/references/core/views/View/#spatialReference) of the view to improve the performance.
 *
 * ### Reference a csv feed URL
 *
 * To create a CSVLayer instance from a csv feed, you must set the [url](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#url) property
 * to the url of the csv feed. If CSV feeds are not on the same domain as the application, a [CORS enabled server](https://developers.arcgis.com/javascript/latest/cors/) or a [proxy](https://developers.arcgis.com/javascript/latest/proxies/)
 * is required.
 *
 * ```js
 ...
 *
 * You can also create a CSVLayer from in memory csv data by passing a [blob url](https://developer.mozilla.org/docs/Web/API/URL/createObjectURL) to the
 * [url](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#url) property of the layer.
 * The following snippet shows how to create a new CSVLayer from a blob url.
 *
 * ```js
 ...
 * > **Notes**
 * >
 * > Starting at version 4.28, date fields are stored in the `UTC` time zone instead of the local time zone of the device. This may cause some date values defined without a time zone offset to be inaccurate.
 * > For example, at version 4.27 and earlier, if your CSV file has date values without the time zone offset such as below, these values would be interpreted in the local time zone of the device.
 * > At 4.28, these values are interpreted in the `UTC` time zone. The following table shows how dates may be interpreted in 4.27 and 4.28.
 * >
 * > | CSV file value | 4.27 and earlier value | 4.28 value |
 * > | -------------- | ---------------------- | ---------- |
 * > | `01/01/2021`     | Interpreted as `1/1/2022, 12:00 AM` in the device's local time zone | `1/1/2022, 12:00 AM` in UTC |
 ...
 * > | `2021-01-01T00:00:00-08:00` | Interpreted as `1/1/2021, 08:00 AM` in UTC time zone | `1/1/2021, 08:00 AM` in UTC time zone |
 * > | `2021-01-01T00:00:00.000Z` | Interpreted as `1/1/2021, 08:00 AM` in UTC time zone | `1/1/2021, 08:00 AM` in UTC time zone |
 * > | `2021-01-01T00:00Z` | Interpreted as `1/1/2021, 08:00 AM` in UTC time zone | `1/1/2021, 08:00 AM` in UTC time zone |
 * >
 ...
 *
 * @since 4.1
 * @see [Sample - Intro to CSVLayer](https://developers.arcgis.com/javascript/latest/sample-code/layers-csv/)
 * @see [Sample - CSVLayer - Project points on the fly](https://developers.arcgis.com/javascript/latest/sample-code/layers-csv-projection/)
...
export default class CSVLayer extends CSVLayerSuperclass {
  ...
  /**
   * The time zone that dates are stored in. Time zone is always `UTC` for CSVLayer.
   ...
   * [latitudeField](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#latitudeField) and [longitudeField](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#longitudeField) columns that represent geographic features.
   * The [geometryType](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#geometryType) of the layer will be `null` if the `isTable` is true.
   *
  ...
  /**
   * Executes a [AttributeBinsQuery](https://developers.arcgis.com/javascript/latest/references/core/rest/support/AttributeBinsQuery/) against a [CSVLayer](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/), which groups features into bins based on ranges in numeric or date fields, and returns a
   ...
   * > [CSVLayerView.queryExtent()](https://developers.arcgis.com/javascript/latest/references/core/views/layers/CSVLayerView/#queryExtent).
   ...
   * > [CSVLayerView.queryFeatureCount()](https://developers.arcgis.com/javascript/latest/references/core/views/layers/CSVLayerView/#queryFeatureCount).
   * > - Spatial queries have the same limitations as those listed in the [projectOperator](https://developers.arcgis.com/javascript/latest/references/core/geometry/operators/projectOperator/) documentation.
   * > - Spatial queries are currently not supported if the layer view has any of the following [SpatialReferences](https://developers.arcgis.com/javascript/latest/references/core/geometry/SpatialReference/):
   * >   - GDM 2000 (4742) - Malaysia
   * >   - Gusterberg (Ferro) (8042) - Austria/Czech Republic
   ...
   * > [CSVLayerView.queryFeatures()](https://developers.arcgis.com/javascript/latest/references/core/views/layers/CSVLayerView/#queryFeatures).
   ...
   * > [CSVLayerView.queryObjectIds()](https://developers.arcgis.com/javascript/latest/references/core/views/layers/CSVLayerView/#queryObjectIds).
   * > - Spatial queries have the same limitations as those listed in the [projectOperator](https://developers.arcgis.com/javascript/latest/references/core/geometry/operators/projectOperator/) documentation.
   * > - Spatial queries are currently not supported if the layer view has any of the following [SpatialReferences](https://developers.arcgis.com/javascript/latest/references/core/geometry/SpatialReference/):
   * >   - GDM 2000 (4742) - Malaysia
   * >   - Gusterberg (Ferro) (8042) - Austria/Czech Republic
   ...
   *
   * @since 4.22
   * @see [refreshInterval](https://developers.arcgis.com/javascript/latest/references/core/layers/CSVLayer/#refreshInterval)
