 ...
 * MapImageLayer allows you to display, query, and analyze layers from data defined in a [map service](https://doc.esri.com/en/arcgis-enterprise/latest/administer/what-is-a-map-service.html).
...
export default class Sublayer extends SublayerSuperclass {
  ...
  /**
   * `effectiveUniqueIdFields` is a convenience property that returns either the sublayer's [uniqueIdFields](https://developers.arcgis.com/javascript/latest/references/core/layers/support/Sublayer/#uniqueIdFields), if defined,
   * or the sublayer's [objectIdField](https://developers.arcgis.com/javascript/latest/references/core/layers/support/Sublayer/#objectIdField) as an array.
   *
   * @since 5.1
   * @example
   * ```js
   * // Create a MapImageLayer and wait for it to load.
   * const layer = new MapImageLayer({
   *   url: "https://sampleserver6.arcgisonline.com/arcgis/rest/services/USA/MapServer",
   * });
   * await layer.load();
   *
   * // Obtain the sublayer with an id of 0.
   * const sublayer = layer.findSublayerById(0);
   *
   * // Use the effectiveUniqueIdFields property to create a query for the sublayer.
   * const query = layer.createQuery();
   * query.outFields = layer.effectiveUniqueIdFields;
   *
   * // Execute the query to obtain the features from the sublayer.
   ...
   * [dynamic sources](https://doc.esri.com/en/arcgis-enterprise/latest/administer/about-dynamic-layers.html).
