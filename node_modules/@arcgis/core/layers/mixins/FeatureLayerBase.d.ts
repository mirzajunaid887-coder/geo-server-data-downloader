...
export abstract class FeatureLayerBase {
  ...
  /**
   * `effectiveUniqueIdFields` is a convenience property that returns either the layer's [FeatureLayer.uniqueIdFields](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/#uniqueIdFields), if defined,
   * or the layer's [FeatureLayer.objectIdField](https://developers.arcgis.com/javascript/latest/references/core/layers/FeatureLayer/#objectIdField) as an array.
   *
   * @since 5.1
   * @example
   * ```js
   * const query = layer.createQuery();
   * query.outFields = layer.effectiveUniqueIdFields;
   * ```
   */
  get effectiveUniqueIdFields(): string[];
  /**
   * Specifies how features are placed on the vertical axis (z). This property may only be used
   * in a [SceneView](https://developers.arcgis.com/javascript/latest/references/core/views/SceneView/). See the [ElevationInfo sample](https://developers.arcgis.com/javascript/latest/sample-code/scene-elevationinfo/)
   * for an example of how this property may be used.
  ...
  /**
   * Indicates whether the layer has m aware geometries.
   *  Use the `supportsM` property in the layer's [DataCapabilities](https://developers.arcgis.com/javascript/latest/references/core/layers/types/#DataCapabilities)
   *  to verify if m-values are supported by the layer.
   *
   *  - When `true`: The layer has m aware features. M-values are part of the geometry.
   *  - When `false`: The layer does not have m aware features. M-values are not part of the geometry.
   *
   * @see [returnM](https://developers.arcgis.com/javascript/latest/references/core/layers/mixins/FeatureLayerBase/#returnM) for details regarding fetching m-values with features from a layer
   * @see [Linear referencing](https://pro.arcgis.com/en/pro-app/latest/help/data/linear-referencing/introduction-to-linear-referencing.htm)
   */
  accessor hasM: boolean;
  /**
   * Indicates whether the layer has z aware geometries. Use the `supportsZ` property in the layer's [DataCapabilities](https://developers.arcgis.com/javascript/latest/references/core/layers/types/#DataCapabilities)
   * to verify if z-values are supported by the layer.
   *
   *  - When `true`: The layer has z aware features. Z-values are part of the geometry.
   *  - When `false`: The layer does not have z aware features. Z-values are not part of the geometry.
   *  > [!NOTE]
   ...
   *
   * @see [returnZ](https://developers.arcgis.com/javascript/latest/references/core/layers/mixins/FeatureLayerBase/#returnZ) for details regarding fetching z-values with features from a layer
   * @see [Point.z](https://developers.arcgis.com/javascript/latest/references/core/geometry/Point/#z) for more information on how z-values are represented in geometries
   */
  ...
  /**
   * Determines whether m-values (measure) are returned in feature geometries. For 2D views, support was added at version 5.1 and the behavior is as follows:
   *  - When `undefined` (default): Fetches m-values if they are needed for rendering or labeling, or layer property interacts with the geometry.
   *  Otherwise, m-values are excluded from feature queries.
   *  - When `true`: Fetches m-values with the geometry, ensuring they are available on the client.
   *  - When `false`: Excludes m-values from feature queries. M-values are not part of geometries.
   *
   *  > [!WARNING]
   *  >
   *  > 3D views do not support m-values. They will not be fetched or returned in 3D scene queries, regardless of this setting.
   *
   * @see [FeatureLayerBase.hasM](https://developers.arcgis.com/javascript/latest/references/core/layers/mixins/FeatureLayerBase/#hasM)
   */
  accessor returnM: boolean | null | undefined;
  /**
   * Determines whether z-values (elevation) are returned in feature geometries. For 3D views, z-values are always fetched regardless of the `returnZ` setting.
   * For 2D views, support was added at version 5.1 and the behavior is as follows:
   * - When `undefined` (default): Fetches z-values if they are needed for rendering or labeling, or layer property interacts with the geometry.
   * Otherwise, z-values are excluded from feature queries.
   * - When `true`: Fetches z-values with the geometry, ensuring they are available on the client.
   * - When `false`: Excludes z-values from feature queries. Z-values are not part of geometries.
   *
   * @see [FeatureLayerBase.hasZ](https://developers.arcgis.com/javascript/latest/references/core/layers/mixins/FeatureLayerBase/#hasZ)
   * @see [Geometry.hasZ](https://developers.arcgis.com/javascript/latest/references/core/geometry/Geometry/#hasZ)
   */
