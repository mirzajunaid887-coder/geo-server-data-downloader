...
export interface ClassBreakInfoProperties extends Partial<Pick<ClassBreakInfo, "label" | "maxValue" | "minValue">> {
  /**
   * Defines alternate CIM symbols used to represent features in this class break at different scales.
   * Each CIMSymbol provided must have a `minScale` or `maxScale` defined.
   *
   * > Alternate symbols are only supported in 2D.
   * > To properly support scale-based symbology, the [symbol](https://developers.arcgis.com/javascript/latest/references/core/renderers/support/ClassBreakInfo/#symbol) property should also be a [CIMSymbol](https://developers.arcgis.com/javascript/latest/references/core/symbols/CIMSymbol/) with a `minScale` or `maxScale` defined.
   *
   * @since 5.1
   * @example
   * ```js
   ...
   *     maxValue: 100,
   *     label: "0-100",
   *     // this symbol will be used when the view's scale is less than 5,000,000
   ...
   *     }),
   *     alternateSymbols: [
   *       // this symbol will be used when the view's scale is greater than 5,000,000
   *       new CIMSymbol({
   *         data: {
   *           type: "CIMSymbolReference",
   *           minScale: 0,
