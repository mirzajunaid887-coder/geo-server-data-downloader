...
import type { LinkChartLayerProperties } from "../../layers/LinkChartLayer.js";
import type { IdTypePair, LayoutMode } from "../../layers/knowledgeGraph/types.js";
...
export interface InclusionModeDefinitionOptimizedGeometry {
  /**
   * Specifies whether to create sublayers for each [named type](https://developers.arcgis.com/javascript/latest/references/core/rest/knowledgeGraph/GraphNamedObject/)
...
/**
 * Uses the layout algorithm library to calculate a layout for a technology-agnostic graph representation of nodes and edges
 * For now, only geometries for node locations are returned
 *
 * @param layoutMode - The layout mode to use for the graph layout calculation.
 * @param nodes - The nodes in the graph to layout.
 * @param links - The links in the graph to layout.
 * @param options - Additional options for the layout calculation.
 * @returns A promise that resolves with the calculated layout, including the extent and locations of the nodes.
 * @internal
 * @internal
 */
export function externalCalculateGraphLayout(layoutMode: LayoutMode, nodes: Array<{
    id: string;
    typeName: string;
    lockedLocation?: Point;
    startTime?: Date;
    endTime?: Date;
    ...
    startTime?: Date;
    endTime?: Date;
}>, options?: AbortOptions & { layoutSettings?: LayoutSettings; }): Promise<{
    extent: {
        xmin: number;
        ymin: number;
        xmax: number;
    ...
    locations: Map<string, Map<string, Point>>;
}>;