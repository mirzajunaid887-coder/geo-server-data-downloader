...
import type RasterInfo from "../../layers/support/RasterInfo.js";
...
export interface ComputeRasterAttributeTableParameters {
  ...
  pixelSize?: Point | null;
  /**
   * When true, only allow downsampling from the source raster resolution.
   *
   * @internal
   */
  downsampleOnly?: boolean;
  /**
   * Maximum requested source level pixel count before downsampling to provided pixel size.
   *
   * @internal
   */
  maxPixelCountForDownsample?: number;
}
...
export interface PixelStatisticsParameters {
  ...
  downsampleOnly?: boolean;
  /**
   * Maximum requested pixel count before forcing the provided pixel size path.
   *
   * @internal
   */
  maxPixelCountForDownsample?: number;
...
/**
 * Get layer's current raster info. To ensure it corresponds to layer.rasterFunction, raster info must be generated beforehand for this sync pattern to work.
 *
 * @param layer - The layer object.
 * @returns the current raster info.
 * @internal
 * @example
 * // during layer initialization, generate raster info (except WCSLayer)
 * await layer.load();
 * await layer.generateRasterInfo(layer.rasterFunction);
 *
 * // later on (as long as layer.rasterFunction is not changed)
 * const rasterInfo = getCurrentRasterInfo(layer);
 * @example
 * // typical async pattern
 * const rasterInfo = await layer.generateRasterInfo(layer.rasterFunction);
 * @internal
 */
export function getCurrentRasterInfo(layer: ImageryLayer | ImageryTileLayer | WCSLayer): RasterInfo | null | undefined;