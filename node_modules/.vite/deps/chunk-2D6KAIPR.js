...
var _n = class _n {
  constructor(i, t, s, m) {
    __publicField(this, "m_EnvelopeType", 2);
    __publicField(this, "xmin");
    __publicField(this, "ymin");
    __publicField(this, "xmax");
    __publicField(this, "ymax");
    void 0 === i ? (this.xmin = Number.NaN, this.ymin = Number.NaN, this.xmax = Number.NaN, this.ymax = Number.NaN) : "number" == typeof i ? (this.xmin = i, this.ymin = t, this.xmax = s, this.ymax = m) : (this.xmin = i.xmin, this.ymin = i.ymin, this.xmax = i.xmax, this.ymax = i.ymax), this.normalize();
  }
  ...
  getCenter() {
    return li.construct(this.getCenterX(), this.getCenterY());
  }
  queryCenter(i) {
  ...
  getLowerLeft() {
    return new li(this.xmin, this.ymin);
  }
  getLowerRight() {
    return new li(this.xmax, this.ymin);
  }
  getUpperLeft() {
    return new li(this.xmin, this.ymax);
  }
  getUpperRight() {
    return new li(this.xmax, this.ymax);
  }
  move(i, t) {
  ...
  clipLine(i, t) {
    ...
    const h = (s ? 1 : 0) | (m ? 2 : 0);
    do {
      const h2 = t.x - i.x, x2 = t.y - i.y;
      if (Math.abs(h2) > Math.abs(x2) ? s & _n.XMASK ? (s & _n.XLESSXMIN ? (i.y += x2 * (this.xmin - i.x) / h2, i.x = this.xmin) : (i.y += x2 * (this.xmax - i.x) / h2, i.x = this.xmax), s = this.clipCode(i)) : m & _n.XMASK ? (m & _n.XLESSXMIN ? (t.y += x2 * (this.xmin - t.x) / h2, t.x = this.xmin) : (t.y += x2 * (this.xmax - t.x) / h2, t.x = this.xmax), m = this.clipCode(t)) : s ? (s & _n.YLESSYMIN ? (i.x += h2 * (this.ymin - i.y) / x2, i.y = this.ymin) : (i.x += h2 * (this.ymax - i.y) / x2, i.y = this.ymax), s = this.clipCode(i)) : (m & _n.YLESSYMIN ? (t.x += h2 * (this.ymin - t.y) / x2, t.y = this.ymin) : (t.x += h2 * (this.ymax - t.y) / x2, t.y = this.ymax), m = this.clipCode(t)) : s & _n.YMASK ? (s & _n.YLESSYMIN ? (i.x += h2 * (this.ymin - i.y) / x2, i.y = this.ymin) : (i.x += h2 * (this.ymax - i.y) / x2, i.y = this.ymax), s = this.clipCode(i)) : m & _n.YMASK ? (m & _n.YLESSYMIN ? (t.x += h2 * (this.ymin - t.y) / x2, t.y = this.ymin) : (t.x += h2 * (this.ymax - t.y) / x2, t.y = this.ymax), m = this.clipCode(t)) : s ? (s & _n.XLESSXMIN ? (i.y += x2 * (this.xmin - i.x) / h2, i.x = this.xmin) : (i.y += x2 * (this.xmax - i.x) / h2, i.x = this.xmax), s = this.clipCode(i)) : (m & _n.XLESSXMIN ? (t.y += x2 * (this.xmin - t.x) / h2, t.x = this.xmin) : (t.y += x2 * (this.xmax - t.x) / h2, t.x = this.xmax), m = this.clipCode(t)), s & m) return 0;
    } while (s | m);
    return h;
  }
  ...
  sqrDistanceEnvelope(i, t, s) {
    ...
    return t && t.setCoords(n2, e), s && s.setCoords(n2, e), m = this.xmin - i.xmax, m > h && (h = m, t && (t.x = this.xmin), s && (s.x = i.xmax)), m = this.ymin - i.ymax, m > x2 && (x2 = m, t && (t.y = this.ymin), s && (s.y = i.ymax)), m = i.xmin - this.xmax, m > h && (h = m, t && (t.x = this.xmax), s && (s.x = i.xmin)), m = i.ymin - this.ymax, m > x2 && (x2 = m, t && (t.y = this.ymax), s && (s.y = i.ymin)), h * h + x2 * x2;
  }
  sqrDistance(i, t) {
    if (void 0 === t) {
  ...
  sqrMaxMinDistanceEnvelope(i) {
    ...
    {
      const m = li.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymin), h = li.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymax), x2 = li.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymin), n2 = li.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymax), e = Math.max(m, h), a = Math.max(x2, n2), r = Math.max(e, a);
      t = Math.min(r, t);
    }
    {
      const m = li.sqrDistanceCoords(this.xmin, this.ymin, i.xmin, i.ymax), h = li.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymax), x2 = li.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymax), n2 = li.sqrDistanceCoords(this.xmax, this.ymin, i.xmax, i.ymax), e = Math.max(m, h), a = Math.max(x2, n2), r = Math.max(e, a);
      t = Math.min(r, t);
    }
    {
      const m = li.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymin), h = li.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymax), x2 = li.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymin), n2 = li.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymax), e = Math.max(m, h), a = Math.max(x2, n2), r = Math.max(e, a);
      t = Math.min(r, t);
    }
    {
      const m = li.sqrDistanceCoords(this.xmin, this.ymax, i.xmin, i.ymin), h = li.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymin), x2 = li.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymin), n2 = li.sqrDistanceCoords(this.xmax, this.ymax, i.xmax, i.ymin), e = Math.max(m, h), a = Math.max(x2, n2), r = Math.max(e, a);
      t = Math.min(r, t);
    }
    return t;
  }
  sqrMaxDistanceEnvelope(i) {
    ...
    for (let m = 0; m < 4; m++) for (let i2 = 0; i2 < 4; i2++) {
      const n2 = li.sqrDistance(h[m], x2[i2]);
      n2 > t && (t = n2);
    }
    return t;
  }
  sqrMaxMinDistance(i) {
    const t = li.sqrDistance(i, this.getLowerLeft()), m = li.sqrDistance(i, this.getUpperLeft()), h = li.sqrDistance(i, this.getLowerRight()), x2 = li.sqrDistance(i, this.getUpperRight());
    let n2, e = Math.max(t, m);
    return n2 = Math.max(t, h), e > n2 && (e = n2), n2 = Math.max(m, x2), e > n2 && (e = n2), n2 = Math.max(h, x2), e > n2 && (e = n2), e;
  }
  sqrMinDistance(i) {
  ...
  sqrMaxDistance(i) {
    let t = li.sqrDistance(i, this.getLowerLeft());
    return t = Math.max(li.sqrDistance(i, this.getUpperLeft()), t), t = Math.max(li.sqrDistance(i, this.getUpperRight()), t), t = Math.max(li.sqrDistance(i, this.getLowerRight()), t), t;
  }
  snapToBoundary(i) {
    ...
    const s = this.xmax - i.x, m = i.x - this.xmin, h = this.ymax - i.y, x2 = i.y - this.ymin;
    return Math.max(s, m) > Math.max(h, x2) ? i.y = x2 < h ? this.ymin : this.ymax : i.x = m < s ? this.xmin : this.xmax, true;
  }
  snapClip(i) {
    const t = Ys(i.x, this.xmin, this.xmax), m = Ys(i.y, this.ymin, this.ymax);
    return li.construct(t, m);
  }
  boundaryDistance(i) {
    if (this.isEmpty() || i.isNAN()) return Number.NaN;
  ...
  calculateToleranceFromEnvelope() {
    ...
    return (Math.abs(this.xmin) + Math.abs(this.xmax) + Math.abs(this.ymin) + Math.abs(this.ymax) + 1) * Os();
  }
  toString() {
    return `[${this.xmin}, ${this.ymin}, ${this.xmax}, ${this.ymax}]`;
  }
};
__publicField(_n, "XLESSXMIN", 1);
__publicField(_n, "YLESSYMIN", 4);
__publicField(_n, "XMASK", 3);
__publicField(_n, "YMASK", 12);
var n = _n;

export {
  n
};
//# sourceMappingURL=chunk-2D6KAIPR.js.map
