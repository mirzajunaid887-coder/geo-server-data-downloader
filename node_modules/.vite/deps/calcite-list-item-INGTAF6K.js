...
} from "./chunk-ZORBDQUZ.js";
import {
  getDepth,
  getListItemChildren,
  listSelector
} from "./chunk-MC2BRHIJ.js";
import "./chunk-7VASGOUH.js";
import "./chunk-HYATQOS4.js";
import "./chunk-B4BDPF4Y.js";
import "./chunk-WDMZLOQR.js";
import "./chunk-5N3CQDZ7.js";
import {
...
import {
  logger
} from "./chunk-WX5YKA4J.js";
import "./chunk-DZO7AAYJ.js";
import "./chunk-TSKPZSMM.js";
import {
  useDirection
...
var _SortHandle = class _SortHandle extends LitElement {
  ...
  renderReorderGroup() {
    return this.hasReorderItems ? keyed("reorder", html`<calcite-dropdown-group .groupTitle=${this.reorderGroupTitle} id=${IDS.reorder} .scale=${this.scale} selection-mode=none>${this.renderTop()}${this.renderUp()}${this.renderDown()}${this.renderBottom()}</calcite-dropdown-group>`) : null;
  }
  renderAddToGroup() {
    const { messages, addToItems, scale, hasAddToItems } = this;
    return hasAddToItems ? keyed("add-to-items", html`<calcite-dropdown-group .groupTitle=${messages.addTo} id=${IDS.add} .scale=${scale} selection-mode=none>${addToItems.map((addToItem) => this.renderAddToItem(addToItem))}</calcite-dropdown-group>`) : null;
  }
  renderMoveToGroup() {
    const { messages, moveToItems, scale, hasMoveToItems } = this;
    return hasMoveToItems ? keyed("move-to-items", html`<calcite-dropdown-group .groupTitle=${messages.moveTo} id=${IDS.move} .scale=${scale} selection-mode=none>${moveToItems.map((moveToItem) => this.renderMoveToItem(moveToItem))}</calcite-dropdown-group>`) : null;
  }
  renderDropdownItem(positionIndex, label, disabled = false) {
    return keyed(REORDER_VALUES[positionIndex], html`<calcite-dropdown-item data-value=${REORDER_VALUES[positionIndex] ?? nothing} .disabled=${disabled} .label=${label} @calciteDropdownItemSelect=${this.handleReorder}>${label}</calcite-dropdown-item>`);
  }
  renderTop() {
...
//# sourceMappingURL=calcite-list-item-INGTAF6K.js.map
