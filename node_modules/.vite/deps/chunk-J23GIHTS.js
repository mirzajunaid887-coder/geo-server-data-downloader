...
function d3(d4, u) {
  const v = d4.vertex;
  d4.include(s), d4.attributes.add("position", "vec3"), d4.vertex.inputs.add("position", () => "position"), d4.attributes.add("normal", "vec3"), u.hasVertexCenterOffset ? d4.attributes.add("centerOffset", "vec3") : v.constants.add("centerOffset", "vec3", [0, 0, 0]), d4.attributes.add("groundDistance", "float"), f(v, u), d(v, u), v.uniforms.add(new e("viewport", (e2) => e2.camera.fullViewport), new r("polygonOffset", (e2) => e2.shaderPolygonOffset), new r2("aboveGround", (e2) => e2.camera.aboveGround ? 1 : -1)), u.hasVerticalOffset && d2(v), v.code.add(t`struct ProjectHUDAux {
vec3 posModel;
vec3 posView;
vec3 vnormal;
float distanceToCamera;
float absCosAngle;
};`), v.code.add(t`float applyHUDViewDependentPolygonOffset(float pointGroundDistance, float absCosAngle, inout vec3 posView) {
...
}`), u.draped && !u.hasVerticalOffset || p(v), u.draped || (v.uniforms.add(new r2("perDistancePixelRatio", (e2) => Math.tan(e2.camera.fovY / 2) / (e2.camera.fullViewport[2] / 2))), v.code.add(t`
...
function l(l2) {
  l2.uniforms.add(new r3("alignPixelEnabled", (e2) => e2.alignPixelEnabled)), l2.code.add(t`vec4 alignToPixelCenter(vec4 clipCoord, vec2 widthHeight) {
...
//# sourceMappingURL=chunk-J23GIHTS.js.map
