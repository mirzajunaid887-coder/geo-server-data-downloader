...
// node_modules/@arcgis/core/views/3d/webgl-engine/core/shaderLibrary/output/ReadDepthLastFrame.glsl.js
function a3(a4) {
  a4.include(a2), a4.uniforms.add(new e2("zProjectionMapLastFrame", (e4) => i(e4.reprojection.lastFrameCamera))), a4.code.add(t`float linearDepthFromTextureLastFrame(sampler2D depthTexture, vec2 uv) {
return linearizeDepth(depthFromTexture(depthTexture, uv), zProjectionMapLastFrame);
}`);
}

// node_modules/@arcgis/core/chunks/GlobalIllumination.glsl.js
var f = 0.01;
var x = 8e-3;
var v = 2e-3;
var M = 0.5;
...
var O = class extends c {
  constructor() {
    super(...arguments), this.projScale = 1, this.scaleGlobalIllumination = 1, this.accumulatedFrames = 0, this.temporalSampleFrame = 0, this.rayMarchMinReach = P, this.rayMarchMaxReach = T, this.rayMarchWorldReach = G, this.rayMarchMinReachEmissionWeight = E, this.rayMarchMaxReachEmissionWeight = $, this.rayMarchMaxSteps = L, this.colorBleedWeight = z;
  }
};
function V(p) {
  const z2 = new s(), G2 = z2.fragment;
  return z2.include(o), z2.include(c2), t3(G2), G2.include(a3), G2.include(a), G2.include(c4), z2.include(c3, p), G2.uniforms.add(new e("normalMap", (e4) => e4.normalTexture), new e("depthMap", (e4) => e4.depthTexture), new e3("lastFrameColorTexture", (e4) => {
    var _a;
    return (_a = e4.reprojection.lastFrameColor) == null ? void 0 : _a.getTexture();
  }), new e3("lastFrameDepthTexture", (e4) => {
    var _a;
    return (_a = e4.reprojection.lastFrameDepth) == null ? void 0 : _a.attachment;
  }), new e3("lastFrameGlobalIlluminationTexture", (e4) => {
    var _a;
    return (_a = e4.globalIllumination) == null ? void 0 : _a.getTexture();
  }), new e3("lastFrameGlobalIlluminationWeightTexture", (e4) => {
    var _a;
    return (_a = e4.globalIllumination) == null ? void 0 : _a.getTexture(C);
  }), new t2("reprojectionViewMatrix", (e4) => e4.reprojection.viewMatrix), new t2("view", (e4) => e4.camera.viewMatrix), new r("accumulatedFrames", (e4) => e4.accumulatedFrames), new r("temporalSampleFrame", (e4) => e4.temporalSampleFrame), new r("scaleGlobalIllumination", (e4) => e4.scaleGlobalIllumination)), G2.uniforms.add(new r("rayMarchMinReach", (e4) => e4.rayMarchMinReach), new r("rayMarchMaxReach", (e4) => e4.rayMarchMaxReach), new r("rayMarchWorldReach", (e4) => e4.rayMarchWorldReach), new r("rayMarchMinReachEmissionWeight", (e4) => e4.rayMarchMinReachEmissionWeight), new r("rayMarchMaxReachEmissionWeight", (e4) => e4.rayMarchMaxReachEmissionWeight), new r("rayMarchMaxSteps", (e4) => e4.rayMarchMaxSteps), new r("colorBleedWeight", (e4) => e4.colorBleedWeight)), p.hasEmission && G2.uniforms.add(new e3("lastFrameEmissionTexture", (e4) => {
    var _a;
    return (_a = e4.reprojection.lastFrameEmission) == null ? void 0 : _a.attachment;
  })), G2.code.add(t`
    float computeIdleColorBlendWeight(float accumulatedFrames) {
      float idleColorBlendProgress = clamp(
        accumulatedFrames / ${t.float(S)},
        0.0,
        1.0
      );
      return mix(
    ...
    float computeIdleOcclusionBlendWeight(float accumulatedFrames) {
      ...
      return mix(
        ${t.float(R)},
        ${t.float(F)},
        pow(idleOcclusionBlendProgress, ${t.float(I)})
      );
    }
  ...
  `), z2.outputs.add("fragGlobalIllumination", "vec4", 0), z2.outputs.add("fragWeight", "float", 1), G2.main.add(t`
    ...
    bool hasReprojectionMismatch = reprojectionDepthMismatch > ${t.float(f)};
    ...
    if (hasReprojectionMismatch) {
      if (isLowQualityEdgePixel) {
        steps = 1;
        occlusionBlendWeight = ${t.float(x)};
      ...
      steps = 1;
      if (historyOcclusionBlendWeight > ${t.float(M)}) {
        occlusionBlendWeight = ${t.float(b)};
        colorBlendWeight = ${t.float(w)};
      } else if (historyOcclusionBlendWeight > ${t.float(y)}) {
        occlusionBlendWeight = historyOcclusionBlendWeight - 0.05;
        colorBlendWeight = ${t.float(w)};
      } else {
        occlusionBlendWeight = isScaledGlobalIllumination ? ${t.float(x)} : idleOcclusionBlendWeight;
        colorBlendWeight = isScaledGlobalIllumination ? ${t.float(v)} : idleColorBlendWeight;
      }
    }
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      if (hit.z > 0.0) {
        ${n(p.hasColor, t`
          ...
          vec3 sourceEmission = ${n(p.hasEmission, "texture(lastFrameEmissionTexture, hitReprojectedCoordinate).xyz", "vec3(0.0)")};
    ...
    float accumulationDither = occlusionBlendWeight < 1.0
      ? randomDirectionSample.b * ${t.float(B)}
      : 0.0;

    ${n(p.hasColor, t`
      // Accumulate color
      vec3 lastFrameColor = lastFrameGlobalIllumination.rgb;
      float colorDitherScale = isScaledGlobalIllumination ? ${t.float(C2)} : 1.0;
...
var U = Object.freeze(Object.defineProperty({ __proto__: null, GlobalIlluminationPassParameters: O, build: V, defaultColorBleedWeight: z, defaultRayMarchMaxReach: T, defaultRayMarchMaxReachEmissionWeight: $, defaultRayMarchMaxSteps: L, defaultRayMarchMinReach: P, defaultRayMarchMinReachEmissionWeight: E, defaultRayMarchWorldReach: G }, Symbol.toStringTag, { value: "Module" }));
...
//# sourceMappingURL=chunk-YE5IPYHR.js.map
