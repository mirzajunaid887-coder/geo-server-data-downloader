    ...
    if (axis) {
      ...
      if (targetCount > 0) {
        if (this.get("syncZeros") && syncMin <= 0 && syncMax >= 0) {
          ...
          let rawStep = 0;
          if (belowCount > 0 && min2 < 0) {
            rawStep = Math.abs(min2) / belowCount;
          }
          if (aboveCount > 0 && max2 > 0) {
            rawStep = Math.max(rawStep, max2 / aboveCount);
          }
          if (rawStep == 0) {
            rawStep = (max2 - min2) / targetCount;
          }
          step = this._niceStep(rawStep);
          min2 = -step * belowCount;
          max2 = step * aboveCount;
        } else {
          let rawStep = (max2 - min2) / targetCount;
          step = this._niceStep(rawStep);
          let newMin = Math.floor(min2 / step) * step;
          let newMax = newMin + step * targetCount;
          if (newMax < max2) {
            newMin = Math.ceil(max2 / step) * step - step * targetCount;
...
var GaplessDateAxis = class extends DateAxis {
  ...
  _updateAllDates() {
    if (!this._customDates) {
      const newDates = /* @__PURE__ */ new Set();
      each(this.series, (series) => {
        if (series.get("ignoreMinMax")) {
          return;
        }
      ...
      if (this._axesDirty || this._valuesDirty || this._stackDirty || this.isDirty("vcx") || this.isDirty("vcy") || this._sizeDirty || this.isDirty("connect") || this.isDirty("curveFactory")) {
        ...
        for (let i = startIndex - 1; i >= 0; i--) {
          ...
          for (let f = 0, flen = this._valueFields.length; f < flen; f++) {
            if (!isNumber(dataItem.get(this._valueFields[f]))) {
              hasValues = false;
              break;
            }
          }
...
//# sourceMappingURL=xyChart-S462CUZV.js.map
