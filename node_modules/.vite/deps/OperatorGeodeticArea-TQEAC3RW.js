import {
  Q
} from "./chunk-RGWN3POY.js";
import "./chunk-ZCWSYXWC.js";
import "./chunk-BTNDEPRL.js";
import {
  w
} from "./chunk-VZPLLJTX.js";
import {
  Al,
  Cc,
  Dc,
...
var Q2 = class {
  constructor(e, t, i, s, n4 = 100) {
    __publicField(this, "m_startPt");
    __publicField(this, "m_endPt");
    __publicField(this, "m_cE2");
    __publicField(this, "m_cE");
    __publicField(this, "m_c1By2e");
    __publicField(this, "m_cRpu");
    __publicField(this, "isPcs");
    __publicField(this, "PEProjcs");
    __publicField(this, "m_points");
...
function V(e, t, i) {
  ...
  const r = new E().execute(e, a2, t, i).getImpl().querySegmentIterator();
  r.stripAttributes();
  const h = ku();
  t.querySpheroidData(h);
  const o = h.e2, m2 = 0 === o ? 2 : 1, p = new li(0, 0), u = new li(0, 0), c = new n2(0), g = new Q2(p, u, t, o, 100);
  for (; r.nextPath(); ) for (; r.hasNextSegment(); ) {
    ...
    const t2 = $n(6, 0, 1, 1e-12, 1e-15, g.makeFunctor());
    c.pe((u.x - p.x) * t2);
  }
...
function K(e, t, i, s) {
  const n4 = new E().execute(e, t, i, s);
  if (null !== n4 && !n4.isEmpty()) {
    const e2 = new n3();
    n4.queryEnvelope(e2);
    const { first: t2, second: a2 } = ie(i, e2, false), _ = Yg(i, t2, null), r = new qm().execute(n4, _, s).calculateArea2D();
...
function te(e, t, i) {
  ...
  if (0 === h) {
    ...
    if (li.distance(i2, s) + 0.5 * e2 > Wt) return null;
  }
  const o = ee[h];
  if (null !== o && o.getGCS().equalHorizontal(e)) return o;
  const m2 = e.getText(), l = r.x, p = r.y, u = 0, E2 = 0;
  let f = 0, P = -1;
  6 !== h ? P = bc.PE_PRJ_LAMBERT_AZIMUTHAL_EQAREA : (n(i), f = 0, P = bc.PE_PRJ_CYLINDRICAL_EQAREA);
  const S = tc("EqualAreaPCS");
  let A;
  P === bc.PE_PRJ_LAMBERT_AZIMUTHAL_EQAREA ? A = `PROJCS["${S}",${m2},PROJECTION["Lambert_Azimuthal_Equal_Area"],PARAMETER["False_Easting",${u}],PARAMETER["False_Northing",${E2}],PARAMETER["Central_Meridian",${l}],PARAMETER["Latitude_of_Origin",${p}],UNIT["Meter",1.0]]` : P === bc.PE_PRJ_CYLINDRICAL_EQAREA ? A = `PROJCS["${S}",${m2},PROJECTION["Cylindrical_Equal_Area"],PARAMETER["False_Easting",${u}],PARAMETER["False_Northing",${E2}],PARAMETER["Central_Meridian",${l}],PARAMETER["Standard_Parallel_1",${f}],PARAMETER["Latitude_of_Origin",${p}],UNIT["Meter",1.0]]` : T("getEqualAreaPcsFixed");
  const x3 = Ru(A);
...
function ie(e, t, i) {
  ...
  const E2 = tc("EqualAreaPCS");
  let f;
  return u === bc.PE_PRJ_LAMBERT_AZIMUTHAL_EQAREA ? f = `PROJCS["${E2}",${a2},PROJECTION["Lambert_Azimuthal_Equal_Area"],PARAMETER["False_Easting",${o}],PARAMETER["False_Northing",${m2}],PARAMETER["Central_Meridian",${r}],PARAMETER["Latitude_of_Origin",${h}],UNIT["Meter",1.0]]` : u === bc.PE_PRJ_ALBERS ? f = `PROJCS["${E2}",${a2},PROJECTION["Albers"],PARAMETER["False_Easting",${o}],PARAMETER["False_Northing",${m2}],PARAMETER["Central_Meridian",${r}],PARAMETER["Standard_Parallel_1",${l}],PARAMETER["Standard_Parallel_2",${p}],PARAMETER["Latitude_of_Origin",${h}],UNIT["Meter",1.0]]` : u === bc.PE_PRJ_CYLINDRICAL_EQAREA ? f = `PROJCS["${E2}",${a2},PROJECTION["Cylindrical_Equal_Area"],PARAMETER["False_Easting",${o}],PARAMETER["False_Northing",${m2}],PARAMETER["Central_Meridian",${r}],PARAMETER["Standard_Parallel_1",${l}],PARAMETER["Latitude_of_Origin",${h}],UNIT["Meter",1.0]]` : T("getEqualAreaPCSInstance"), $t(Ru(f), true);
}
function se(e, t, i) {
...
var ne = class {
  ...
  execute(s, n4, _) {
    if (0 === n4.getCoordinateSystemType() && M(""), s.isEmpty() || s.getDimension() < 2) return 0;
    ...
    const u = n4.getGCS();
    u !== n4 && (p = Yg(n4, u));
    let c = new Em().execute(l, n4, false, _);
    return c.isEmpty() ? 0 : (c === s && (c = s.clone()), n4.isPannable() ? V(c, n4, _) : Z(c, n4, u, p, _));
  }
};
function ae() {
  return { m_p_PCS: new li(), m_factor: Number.NaN, setValues: _e, assign: re };
}
function _e(e, t) {
...
var he = class {
  constructor(e, t, i, s) {
    __publicField(this, "m_ptStart", new X());
    __publicField(this, "m_ptEnd", new X());
    __publicField(this, "m_deltaX");
    __publicField(this, "m_deltaY");
    __publicField(this, "m_e");
    __publicField(this, "m_e2");
    __publicField(this, "m_c1MinusE2");
    __publicField(this, "m_baseA");
    this.m_ptStart.assign(t), this.m_ptEnd.assign(i), this.m_deltaX = this.m_ptEnd.x - this.m_ptStart.x, this.m_deltaY = this.m_ptEnd.y - this.m_ptStart.y, this.m_e = Math.sqrt(e), this.m_e2 = e, this.m_c1MinusE2 = 1 - e;
    const n4 = Math.sin(s);
    let a2;
...
var ge = class {
  constructor(t, i, s) {
    ...
    this.m_progressTracker = s, n(4 !== i), this.m_curveType = i, this.m_inputSR = t, t && 0 !== t.getCoordinateSystemType() || M(""), this.m_inputGCS = t.getGCS(), this.m_a = 0, this.m_eSquared = 0, this.m_b = 0, this.m_rpu = 0;
  }
  executePolygonGeodeticArea(e) {
    ...
    if (this.m_inputSR.isPannable()) {
      ...
      if (!t2.containsEnvelope(e2)) {
        ...
        const _ = new x();
        e2.queryIntervalX(_), i = a2.contains(_) ? Al(i, this.m_inputSR) : new qm().foldInto360RangeGeodetic(i, this.m_inputSR, this.m_curveType);
      }
    }
    if (this.m_transformPCS2GCS ? (i = new Em().execute(i, this.m_inputSR, false, this.m_progressTracker), t = i.createInstance(), Zl(this.m_transformPCS2GCS, i, t, this.m_progressTracker) || (t = new qm().execute(i, this.m_transformPCS2GCS, this.m_progressTracker))) : t = new Em().execute(i, this.m_inputGCS, false, this.m_progressTracker), t.isEmpty()) return 0;
  ...
  executeClippedPolygonGeodeticArea(e, t, i) {
    ...
    try {
      ...
      zl(a2, _);
      const r = 50, h = _.getImpl().getAttributeStreamRef(0), o = e.getImpl().getAttributeStreamRef(0), m2 = __addDisposableResource(s, new Cc(), false), l = __addDisposableResource(s, new Cc(), false), u = 40, c = [0, 0], g = Ut(ae, u), d = Yt(u, -1), E2 = ae(), f = ae();
      let P, y;
      const R = a2;
      let M2 = _.calculateArea2D();
      const C2 = new n2(0), N = 0.5 * Math.PI, b = 1e-10 * Math.abs(M2) + 1e-6, I = e.getPathCount();
      let v, D, G, O;
      const k = new li(), F = new li(), J = new li(), Y = new li(), U = new li(), j3 = new li();
      let B2, H2;
      for (D = e.getPathStart(0), v = 0; v < I; v++, D = G) for (G = e.getPathEnd(v), h.queryPoint2D(G - 1 << 1, J), o.queryPoint2D(G - 1 << 1, k), Math.abs(k.y) > N && (k.y = B(N, k.y)), O = D; O < G; O++, J.assign(Y), k.assign(F)) {
        if (h.queryPoint2D(O << 1, Y), o.queryPoint2D(O << 1, F), Math.abs(F.y) > N && (F.y = B(N, F.y)), B2 = li.distance(J, Y), B2 < r || 0 === k.y && 0 === F.y) continue;
        Dc.geodeticDistance(this.m_a, this.m_eSquared, k.x, k.y, F.x, F.y, m2, l, null, this.m_curveType);
        const e2 = m2.val, t2 = l.val;
        for (E2.setValues(0, J), f.setValues(1, Y), y = i, g[0].assign(f), d[0] = i, P = 0; P >= 0; ) {
          H2 = 0.5 * (E2.m_factor + f.m_factor), Dc.geodeticCoordinate(this.m_a, this.m_eSquared, k.x, k.y, e2 * H2, t2, m2, l, this.m_curveType), U.x = m2.val, U.y = l.val, c[0] = U.x, c[1] = U.y, wc.geogToProj(R, 1, c), j3.x = c[0], j3.y = c[1];
          const i2 = -j3.offset(E2.m_p_PCS, f.m_p_PCS), s2 = 0.5 * i2 * li.distance(E2.m_p_PCS, f.m_p_PCS);
          if (C2.pe(s2), Math.abs(s2) > b || Math.abs(s2) > 0 && y > 0) f.setValues(H2, j3), P++, g[P].assign(f), Math.abs(s2) <= b ? (y--, d[P - 1] = y, d[P] = y) : (y = d[P - 1], d[P] = y);
          else {
            if (P <= 0) break;
            E2.assign(f), P--, f.assign(g[P]), y = d[P];
          }
        }
      }
  ...
  executeClippedPolygonGreatEllipticArea(e) {
    const t = [], i = [], s = new li(), n4 = new li(), a2 = ds(1, this.m_eSquared, new li(0, 0.5 * Math.PI)), _ = new n2(0), r = new n2(0), h = new n2(0), o = new X(), m2 = new X(), l = new n3();
    ...
    for (g.stripAttributes(); g.nextPath(); ) for (; g.hasNextSegment(); ) {
      ...
      for (let t2 of i) {
        ...
        const n5 = ds(1, this.m_eSquared, e3), _2 = ds(1, this.m_eSquared, i2), o2 = Math.min(e3.y, i2.y), m3 = Hs(this.m_eSquared, e3.x, i2.x, s2, o2);
        r.pe(m3);
        const l2 = Math.abs(Hs(this.m_eSquared, e3.x, i2.x, o2, Wt)), u2 = new X();
        u2.setSub(n5, a2);
        const g2 = new X();
        g2.setSub(_2, a2);
        const d = new X();
        d.setCrossProductVector(u2, g2);
        let E2 = c * (l2 - 0.5 * d.length());
        E2 = B(E2, i2.x - e3.x), h.pe(E2);
      }
  ...
  getEqualAreaPCSInstance(e, t) {
    ...
    return o = h ? Ic.fromString(bc.PE_TYPE_PROJCS, `PROJCS["${m2}", ${l}, PROJECTION["Lambert_Azimuthal_Equal_Area"],PARAMETER["False_Easting",${s}],PARAMETER["False_Northing", ${n4}],PARAMETER["Central_Meridian", ${i}], PARAMETER["Latitude_of_Origin", ${a2}], UNIT["Meter",1.0]]`, true) : Ic.fromString(bc.PE_TYPE_PROJCS, `PROJCS["${m2}", ${l}, PROJECTION["Cylindrical_Equal_Area"],PARAMETER["False_Easting", ${s}],PARAMETER["False_Northing", ${n4}],PARAMETER["Central_Meridian", ${i}],PARAMETER["Standard_Parallel_1", ${_}],PARAMETER["Latitude_of_Origin", ${a2}],UNIT["Meter",1.0]]`, true), o;
  }
  splitSegmentCrossingItegralThreshold(e, t, i) {
    const s = 100, n4 = 20, a2 = n4 * n4, _ = ds(this.m_a, this.m_eSquared, new li(0, Wt)), h = ds(this.m_a, this.m_eSquared, new li(0, -Wt)), o = this.splitSegmentPassingThroughPole(e);
    for (let m2 of o) {
      const e2 = m2.getStartXY(), o2 = m2.getEndXY(), l = ds(this.m_a, this.m_eSquared, e2), p = ds(this.m_a, this.m_eSquared, o2), u = new li(), c = new X();
      let g = X.sqrDistance(_, l), d = X.sqrDistance(l, p);
      if (g <= s) {
        if (!(d > a2)) {
          i.push(new Pu({ start: e2, end: o2 }));
          continue;
        }
        c.assign(p.sub(l)), c.normalizeThis(), c.assign(l.add(c.mul(n4))), u.assign(Ps(this.m_a, this.m_eSquared, c)), i.push(new Pu({ start: e2, end: u })), e2.assign(u);
      }
      if (l.assign(ds(this.m_a, this.m_eSquared, e2)), g = X.sqrDistance(_, p), d = X.sqrDistance(l, p), g <= s) {
        ...
        c.assign(p.sub(l)), c.normalizeThis(), c.assign(p.sub(c.mul(n4))), u.assign(Ps(this.m_a, this.m_eSquared, c)), i.push(new Pu({ start: u, end: o2 })), o2.assign(u);
      }
      ...
      t.push(new Pu({ start: e2, end: o2 }));
    }
  }
  splitSegmentPassingThroughPole(e) {
    const t = { stack: [], error: void 0, hasError: false };
    try {
      const i = [], s = new X(), n4 = new X(), a2 = new X(), _ = new li(), h = 20, o = 3.124139361, m2 = 10, l = e.getStartXY(), p = e.getEndXY(), u = __addDisposableResource(t, new Cc(), false);
      if (Dc.geodeticDistance(this.m_a, this.m_eSquared, l.x, l.y, p.x, p.y, u, null, null, 2), Math.abs(p.x - l.x) > o && u.val > h) {
        const e2 = new Q(new li(0, 90), l.divide(this.m_rpu), p.divide(this.m_rpu), this.m_inputGCS, 2, 2);
        let t2 = cs(e2.makeFunctor(), 0, 1, 1e-10);
        if (t2.second <= m2 && t2.first > 0 && t2.first < 1) return s.assign(ds(this.m_a, this.m_eSquared, l)), n4.assign(ds(this.m_a, this.m_eSquared, p)), j2(s, n4, t2.first, a2), _.assign(Ps(this.m_a, this.m_eSquared, a2)), i.push(new Pu({ start: l, end: _ })), i.push(new Pu({ start: _, end: p })), i;
        if (e2.setPointDistFrom(new li(0, -90)), t2 = cs(e2.makeFunctor(), 0, 1, 1e-10), t2.second <= m2 && t2.first > 0 && t2.first < 1) return s.assign(ds(this.m_a, this.m_eSquared, l)), n4.assign(ds(this.m_a, this.m_eSquared, p)), j2(s, n4, t2.first, a2), _.assign(Ps(this.m_a, this.m_eSquared, a2)), i.push(new Pu({ start: l, end: _ })), i.push(new Pu({ start: _, end: p })), i;
      }
      return i.push(new Pu({ start: l, end: p })), i;
    } catch (i) {
  ...
  loxodromeArea(e) {
    ...
    const _ = e.getPathCount(), r = new li(), h = new li(), o = e.getImpl().getAttributeStreamRef(0);
  ...
  calculate(e) {
    ...
    return this.m_inputSR !== this.m_inputGCS && null === this.m_transformPCS2GCS && (this.m_transformPCS2GCS = Yg(this.m_inputSR, this.m_inputGCS, null)), this.executePolygonGeodeticArea(e);
  }
};
var de = class {
  getOperatorType() {
...
//# sourceMappingURL=OperatorGeodeticArea-TQEAC3RW.js.map
