...
function t3(n2) {
  n2.uniforms.add(new e("mainLightDirection", (i) => i.lighting.mainLight.direction));
}
function o(n2) {
  n2.uniforms.add(new e("mainLightIntensity", (i) => i.lighting.mainLight.intensity));
}
function a2(i) {
  t3(i), o(i), i.code.add(t`vec3 applyShading(vec3 shadingNormal, float shadow) {
float dotVal = clamp(dot(shadingNormal, mainLightDirection), 0.0, 1.0);
return mainLightIntensity * ((1.0 - shadow) * dotVal);
}`);
}

// node_modules/@arcgis/core/views/3d/webgl-engine/core/shaderLibrary/shading/ScreenSpaceRayMarching.glsl.js
function c(c2, d) {
  const s = c2.fragment;
  s.include(a), s.uniforms.add(new e2("nearFar", (e4) => e4.camera.nearFar), new e3("depthMap", (e4) => {
    var _a;
    return (_a = e4.depth) == null ? void 0 : _a.attachment;
  }), new t2("proj", (e4) => e4.camera.projectionMatrix), new r("invResolutionHeight", (e4) => 1 / e4.camera.height), new t2("reprojectionMatrix", (e4) => e4.reprojection.matrix)).code.add(t`
  ...
  float rayMarchScreenReachFromWorldReach(vec3 startPosition, vec3 rayDirection, float rayMarchWorldReach) {
    ...
    return ${d.useProjectedRayLength ? "length(projectedCoordOffset)" : "abs(projectedCoordOffset.y)"};
  }
  ...
  vec3 screenSpaceIntersectionWithLimits(
    ...
    float rayMarchDistance = ${d.useProjectedRayLength ? "length(vanishingPointScreenOffset.xy)" : "abs(vanishingPointScreenOffset.y)"};
    ...
    for(int stepIndex = 0; stepIndex < rayMarchMaxStepsInt - 1; ++stepIndex) {
      ...
      ${n(!d.clampRayToScreen, "if (projectedPosition.x <= 0.0  || projectedPosition.x >= 1.0) {\n        return vec3(projectedPosition, 0.0);\n      }")}
      ...
      ${n(d.clampRayToScreen, "projectedPosition = clamp(projectedPosition, vec2(0.0), vec2(0.999));")}
    }
    return vec3(projectedPosition, 0.0);
  }
  ...
  vec3 screenSpaceIntersection(vec3 rayDirection, vec3 startPosition, vec3 viewDirection, vec3 normal, float rayStepOffset) {
    return screenSpaceIntersectionWithLimits(
      ...
      normal,
      rayStepOffset,
      ${t.float(d.rayMarchMaxReach)},
      ${t.float(d.rayMarchMaxSteps)}
    );
  }
  `);
}
...
//# sourceMappingURL=chunk-WQFSH25T.js.map
