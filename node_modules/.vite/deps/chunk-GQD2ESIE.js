...
var f = class _f {
  static async create(t2, e3) {
    const r3 = await g2(t2, e3);
  ...
  destroy() {
    var _a;
    (_a = this._mesh) == null ? void 0 : _a.destroy();
  }
  getMesh(t2, e3) {
    var _a;
    const r3 = this._target;
    if (!r3) return null;
    if (null !== this._mesh && this._lastStateVersion === e3.id) return this._mesh;
    const { vertices: s6, indices: i3 } = r3.getTriangulation(e3), n8 = i3 ? new Uint32Array(i3) : void 0, o2 = new Int32Array(s6);
    return this._lastStateVersion = e3.id, (_a = this._mesh) == null ? void 0 : _a.destroy(), this._mesh = s3.createForShader(t2, { primitive: _.TRIANGLES, index: n8, vertex: o2, layout: { pos: { type: R2.INT, count: 2 } } }), this._mesh;
  }
};
...
var _2 = class __ {
  static async fromGeometry({ geometry: t2 }, e3) {
    if (!t2) return null;
    switch (t2.type) {
      case "polygon":
        return this._fromPolygon(t2, e3);
      case "extent":
        return this._fromMapExtent(t2);
    }
  }
  static async _fromPolygon(t2, e3) {
    const s6 = t2.spatialReference ?? e3;
    await p(s6, e3);
    const c3 = H(t2, e3), l3 = ae(e3), p2 = R(c3, false, false), h2 = rt(p2, "esriGeometryPolygon", 100 * l3, false, false);
    if (!h2) return null;
    const u3 = await a2(h2);
    return u3 ? new __(u3.vertices, u3.indices) : null;
  }
  static _fromMapExtent(t2) {
    const { xmin: e3, ymin: r3, xmax: s6, ymax: i3 } = t2, n8 = new Float32Array([e3, r3, s6, r3, e3, i3, e3, i3, s6, r3, s6, i3]), o2 = new Uint32Array([0, 1, 2, 3, 4, 5]);
    return new __(n8, o2);
  }
  constructor(t2, e3) {
    this._vertices = t2, this._indices = e3;
  }
  getTriangulation(t2) {
    const e3 = [], r3 = n4();
    for (let s6 = 0; s6 < this._vertices.length / 2; s6++) {
      const [i3, n8] = t2.toScreen(r3, [this._vertices[2 * s6], this._vertices[2 * s6 + 1]]);
      e3.push(i3, n8);
    }
    return { vertices: e3, indices: this._indices };
  }
};
var d = class _d {
  static fromPath({ path: t2 }) {
    const e3 = O({ paths: t2 }, false, false), r3 = g(e3);
...
var w2 = class {
  ...
  getTriangulation(t2) {
    const { xmin: e3, xmax: r3, ymin: s6, ymax: i3 } = a3(this._rect, t2.size[0], t2.size[1]);
    return { vertices: [e3, s6, r3, s6, e3, i3, e3, i3, r3, s6, r3, i3], indices: new Uint32Array([0, 1, 2, 3, 4, 5]) };
  }
};
...
var i = class extends b {
  constructor() {
    super(...arguments), this._idToCounters = new e();
  }
  get size() {
  ...
  add(e3, t2) {
    let s6 = false;
    for (const r3 of e3) {
      const e4 = o(this._idToCounters, r3, () => (s6 = true, /* @__PURE__ */ new Map())), i3 = e4.get(t2) ?? 0;
      i3 || (s6 = true), e4.set(t2, i3 + 1);
    }
    return s6;
  }
  delete(e3, t2) {
    let o2 = false;
    for (const s6 of e3) {
      const e4 = this._idToCounters.get(s6);
      if (!e4) continue;
      let r3 = e4.get(t2);
      null != r3 && (r3--, r3 > 0 ? e4.set(t2, r3) : (e4.delete(t2), o2 = true), 0 === e4.size && (this._idToCounters.delete(s6), o2 = true));
    }
    return o2;
  }
};
function* n6(e3) {
  for (const [t2, o2] of e3) yield [t2, o2.keys()];
}
...
var n7 = y = class extends t {
  constructor(e3) {
    super(e3), this.type = "geometry", this.geometry = null;
  }
  clone() {
    var _a;
...
var _3 = q.ofType({ key: "type", base: null, typeMap: { rect: s4, path: s5, geometry: n7 } });
var H2 = new (q.ofType(u2))();
var S = (f2) => {
  const m = f2;
  let y2 = class extends m {
    constructor() {
    ...
    initialize() {
      var _a, _b, _c;
      const t2 = ((_a = this.view) == null ? void 0 : _a.spatialReferenceLocked) ?? true, e3 = (_b = this.view) == null ? void 0 : _b.spatialReference;
      e3 && t2 && !this.spatialReferenceSupported ? this.addResolvingPromise(Promise.reject(new r("layerview:spatial-reference-incompatible", "The spatial reference of this layer does not meet the requirements of the view", { layer: this.layer }))) : (this.container || (this.container = new n5()), this.container.fadeTransitionEnabled = true, this.container.visible = false, this.container.endTransitions(), this.addHandles([l(() => this.suspended, (t3) => {
        ...
        if (this.container) {
          const t3 = this.view.spatialReference, e4 = await this._updatingHandles.addPromise(Promise.all(this.clips.items.map((e5) => f.create(e5, t3))));
          for (const i3 of this.container.clips) i3.destroy();
          this.container.clips = e4.filter((t4) => !!t4);
        }
      }, w), l(() => {
        var _a2;
      ...
      }, ({ scale: t3, scaleRange: e4 }) => {
        const i3 = r2(e4, t3);
        i3 !== this._visibleAtCurrentScale && (this._visibleAtCurrentScale = i3);
      }, w), l(() => {
        var _a2;
        return (_a2 = this.layer) == null ? void 0 : _a2.effectiveVisible;
      }, async (t3) => {
        this.container.requestDeclutter();
      })], "constructor"), ((_c = this.view) == null ? void 0 : _c.whenLayerView) ? this.view.whenLayerView(this.layer).then((t3) => {
    ...
    destroy() {
      ...
      for (const t2 of ((_a = this.container) == null ? void 0 : _a.clips) ?? []) t2.destroy();
    }
    get highlightOptions() {
      var _a;
      return this._logHighlightOptionsDeprecation(), (_a = this.highlights) == null ? void 0 : _a.find(({ name: t2 }) => t2 === c2);
    }
    set highlightOptions(t2) {
      ...
      t2 ? i3 == null ? void 0 : i3.assignFrom(t2) : (this.highlights.remove(i3), 0 === this.highlights.length && (this.highlights = null));
    }
    ...
    get _mergedHighlights() {
      ...
      for (const e3 of this.highlights) {
        const i3 = t2.find((t3) => t3.name === e3.name);
        i3 && i3.assignFrom(e3);
      }
      return t2;
    }
    ...
    get spatialReferenceSupported() {
      var _a;
      const t2 = (_a = this.view) == null ? void 0 : _a.spatialReference;
    ...
    get updating() {
      ...
      return this.updateRequested || this.isUpdating() || ((_a = this._updatingHandles) == null ? void 0 : _a.updating) || this.container.transitioning;
    }
    ...
    getSuspendInfo() {
      const t2 = super.getSuspendInfo(), e3 = !this.spatialReferenceSupported;
      return e3 && (t2.spatialReferenceNotSupported = e3), t2;
    }
    addAttachHandles(t2) {
      this.addHandles(t2, "attach");
    }
    _addHighlights(t2, e3) {
      this._highlightCounter.add(t2, e3) && this._processHighlight();
    }
    _removeHighlights(t2, e3) {
      this._highlightCounter.delete(t2, e3) && this._processHighlight();
    }
    _processHighlight() {
    }
    _getHighlights() {
      ...
      for (const [e3, i3] of this._highlightCounter.highlightNamesByObjectId) {
        ...
        t2.push({ objectId: e3, highlightFlags: s6 });
      }
      return t2;
    }
    _getHighlightBits(t2) {
      ...
      const r3 = this._mergedHighlights;
      for (const { name: h2 } of r3) e3.delete(h2) && (s6 = i3), i3 <<= 1;
      return s6;
    }
  };
  return __decorate([a()], y2.prototype, "attached", void 0), __decorate([a({ type: _3, set(t2) {
    const e3 = n2(t2, this._get("clips"), _3);
    this._set("clips", e3);
...
//# sourceMappingURL=chunk-GQD2ESIE.js.map
