...
var _ = class {
  static fromOptimized(t4, e2, s5 = 1) {
    return new u2().initialize(t4, e2, s5);
  }
  static fromJSON(t4) {
    const [e2, h2] = c(t4), r = l(t4), n2 = s3(t4);
    return new l2().initialize(e2, h2, r, n2, 1);
  }
  static fromOptimizedCIM(t4, e2, s5 = 1) {
    return new g2().initialize(t4, e2, s5);
  }
  static fromJSONCIM(t4, e2 = 1) {
    const [h2, r] = c(t4), n2 = l(t4), o2 = s3(t4);
    return new f().initialize(h2, r, n2, o2, e2);
  }
  static fromFeatureSetReader(t4) {
    const e2 = t4.readGeometryForDisplay(), s5 = t4.geometryType;
    return e2 && s5 ? this.fromOptimized(e2, s5) : null;
  }
  static fromFeatureSetReaderCIM(t4) {
    const e2 = t4.readGeometryForDisplay(), s5 = t4.geometryType;
    return e2 && s5 ? this.fromOptimizedCIM(e2, s5) : null;
  }
  static createEmptyOptimized(t4, e2 = false, s5 = false, i2 = 1) {
    return new g2().initialize(new s4([], [], e2, s5), t4, i2);
  }
  static createEmptyJSON(t4, e2 = false, s5 = false) {
    return new l2().initialize([], t4, e2, s5, 1);
  }
  static createEmptyOptimizedCIM(t4, e2 = false, s5 = false, i2 = 1) {
    return new g2().initialize(new s4([], [], e2, s5), t4, i2);
  }
  static createEmptyJSONCIM(t4, e2 = false, s5 = false, i2 = 1) {
    return new f().initialize([], t4, e2, s5, i2);
  }
  asJSON() {
    const t4 = t(this), { hasZ: s5, hasM: i2 } = this;
    return "esriGeometryEnvelope" === this.geometryType ? { xmin: t4[0][0][0], ymin: t4[0][0][1], xmax: t4[0][2][0], ymax: t4[0][2][1] } : "esriGeometryMultipoint" === this.geometryType ? { points: t4.flat(), hasZ: s5, hasM: i2 } : "esriGeometryPoint" === this.geometryType ? { x: t4[0][0][0], y: t4[0][0][1], z: s5 ? t4[0][0][2] : void 0, m: i2 ? t4[0][0][s5 ? 3 : 2] : void 0 } : "esriGeometryPolygon" === this.geometryType ? { rings: t4, hasZ: s5, hasM: i2 } : { paths: t4, hasZ: s5, hasM: i2 };
  }
  getCurrentRingArea() {
    ...
    t4 = this.x, e2 = this.y;
    const i2 = t4, h2 = e2;
    for (; this.nextPoint(); ) s5 += (t4 - this.x) * (e2 + this.y), t4 = this.x, e2 = this.y;
    return s5 += (t4 - i2) * (e2 + h2), -0.5 * s5;
  }
  invertY() {
    this.yFactor *= -1;
  }
};
var u2 = class _u extends _ {
  constructor() {
    super(...arguments), this._end = -1;
  }
  get hasZ() {
    var _a;
    return (_a = this._geometry) == null ? void 0 : _a.hasZ;
  }
  get hasM() {
    var _a;
    return (_a = this._geometry) == null ? void 0 : _a.hasM;
  }
  initialize(t4, e2, s5) {
    return this.geometryType = e2, this._stride = t4.stride, this._geometry = t4, this._pathIndex = -1, this._pathOffset = 0, this._pointOffset = -this._stride, this._end = -1, this.yFactor = s5, this;
  }
  reset() {
  ...
  getCurrentPath() {
    const t4 = this._end, e2 = this._geometry.coords, s5 = this._pathOffset;
    return new _u().initialize(new s4([this.pathSize], e2.slice(s5, t4), this.hasZ, this.hasM), this.geometryType, this.yFactor);
  }
  pathLength() {
    const t4 = this._end, e2 = this._stride, s5 = this._geometry.coords;
    let i2 = 0;
    for (let h2 = this._pathOffset + e2; h2 < t4; h2 += e2) {
      const t5 = s5[h2 - e2], r = s5[h2 - e2 + 1], n2 = s5[h2] - t5, o2 = s5[h2 + 1] - r;
  ...
  isClosed() {
    const t4 = this._geometry.coords, e2 = this._pathOffset, s5 = this._end - this._stride;
    for (let i2 = 0; i2 < this._stride; i2++) if (t4[e2 + i2] !== t4[s5 + i2]) return false;
    return true;
  }
  clone() {
...
var l2 = class _l extends _ {
  initialize(t4, e2, s5, i2, h2) {
    return this._paths = t4, this.geometryType = e2, this.hasZ = s5, this.hasM = i2, this._pathIndex = this._pointIndex = -1, this.yFactor = h2, this._mIndex = this.hasZ ? 3 : 2, this;
  }
  reset() {
  ...
  pushCursor(t4) {
    const s5 = t(t4);
    for (const e2 of s5) this.pushPath(e2);
  }
  asOptimized() {
    const { hasZ: t4, hasM: e2 } = this, s5 = new s4([], [], t4, e2), { coords: i2, lengths: h2 } = s5;
    if ("esriGeometryPoint" === this.geometryType) i2.push(...this._paths[0][0]), h2.length = 0;
    else for (const r of this._paths) {
      for (const s6 of r) i2.push(s6[0]), i2.push(s6[1] * this.yFactor), t4 && i2.push(s6[2]), e2 && i2.push(s6[this._mIndex]);
      h2.push(r.length);
    }
    return s5;
  }
  isClosed() {
    ...
    for (let s5 = 0; s5 < t4.length; s5++) if (t4[s5] !== e2[s5]) return false;
    return true;
  }
  clone() {
...
var y = 4;
var m = 1;
var g2 = class _g extends u2 {
  initialize(t4, e2, s5) {
    return super.initialize(t4, e2, s5), this._controlPoints || (this._controlPoints = this._controlPoints = new Array(this.totalSize).fill(void 0).map((t5) => /* @__PURE__ */ new Set())), this;
  }
  startPath() {
...
var f = class _f extends l2 {
  initialize(t4, e2, s5, i2, h2) {
    return super.initialize(t4, e2, s5, i2, h2);
  }
  clone() {
...
function l3(t4) {
  ...
  if (t4.seekPath(s5), 0 === t4.pathSize) return null;
  t4.seekPathStart();
  const l4 = g(t4);
  if (Math.abs(c2) <= 2 * o2 * o2) return [(l4[0] + l4[2]) / 2, (l4[1] + l4[3]) / 2];
  t4.seekPathStart();
  const x3 = s2(t4, u());
  ...
  const b2 = [NaN, NaN];
  let S = false, k = 0.25, z = -1, g3 = NaN;
  do {
    if (g3 = NaN, m3[1] = h(t4, w(l4[0], l4[2], k), o2, a2), isNaN(m3[1][0]) || isNaN(m3[1][1]) || (M2 = f2(m3[1], t4, false), g3 = M2.distance), !isNaN(g3) && g3 > o2 && u4(m3[1], t4)) S = true, d3[1] = g3, P2[1] = p2(m3[1], x3);
    else if (!isNaN(g3) && g3 > z && (z = g3, b2[0] = m3[1][0], b2[1] = m3[1][1]), k -= 0.01, k < 0.1) {
      if (!(z >= 0)) break;
      S = true, d3[1] = z, m3[1][0] = b2[0], m3[1][1] = b2[1], P2[1] = p2(m3[1], x3);
    }
  } while (!S);
  S = false, k = 0.5, z = -1;
  let q = 0.01, j = 1;
  do {
    if (g3 = NaN, m3[2] = h(t4, w(l4[0], l4[2], k), o2, a2), isNaN(m3[2][0]) || isNaN(m3[2][1]) || (M2 = f2(m3[2], t4, false), g3 = M2.distance), !isNaN(g3) && g3 > o2 && u4(m3[2], t4)) S = true, d3[2] = g3, P2[2] = p2(m3[2], x3);
    else if (!isNaN(g3) && g3 > z) z = g3, b2[0] = m3[2][0], b2[1] = m3[2][1];
    else if (g3 > z && (z = g3, b2[0] = m3[2][0], b2[1] = m3[2][1]), k = 0.5 + q * j, q += 0.01, j *= -1, k < 0.3 || k > 0.7) {
      if (!(z >= 0)) break;
      S = true, d3[2] = z, m3[2][0] = b2[0], m3[2][1] = b2[1], P2[2] = p2(m3[2], x3);
    }
  } while (!S);
  S = false, k = 0.75, z = -1;
  do {
    if (g3 = NaN, m3[3] = h(t4, w(l4[0], l4[2], k), o2, a2), isNaN(m3[3][0]) || isNaN(m3[3][1]) || (M2 = f2(m3[3], t4, false), g3 = M2.distance), !isNaN(g3) && g3 > o2 && u4(m3[3], t4)) S = true, d3[3] = g3, P2[3] = p2(m3[3], x3);
    else if (g3 > z && (z = g3, b2[0] = m3[3][0], b2[1] = m3[3][1]), k += 0.01, k > 0.9) {
      if (!(z >= 0)) break;
      S = true, d3[3] = z, m3[3][0] = b2[0], m3[3][1] = b2[1], P2[3] = p2(m3[3], x3);
    }
  } while (!S);
...
function f2(t4, e2, n2) {
  ...
  let i2 = 1 / 0, r = 0, a2 = 0, s5 = [0, 0], c2 = [0, 0];
  const N2 = [0, 0];
  for (e2.reset(); e2.nextPath() && e2.nextPoint(); ) if (!(e2.pathSize < 2)) for (s5[0] = e2.x, s5[1] = e2.y; e2.nextPoint(); s5 = c2) {
    c2 = [e2.x, e2.y], s(N2, t4, s5, c2);
    const n3 = p2(t4, N2);
...
function h(t4, n2, i2, r) {
  const a2 = [n2, 0];
  let o2 = 1 / 0, s5 = 1 / 0, N2 = false, l4 = false;
  ...
  for (t4.reset(); t4.nextPath() && t4.nextPoint(); ) if (!(t4.pathSize < 2)) for (d3[0][0] = t4.x, d3[0][1] = t4.y; t4.nextPoint(); d3[0][0] = d3[1][0], d3[0][1] = d3[1][1]) {
    ...
    if (!p(u5[0], u5[1], d3[0], d3[1], f3)) continue;
    const e2 = f3[1];
    o2 > s5 ? e2 < o2 && (o2 = e2, N2 = true) : e2 < s5 && (s5 = e2, l4 = true);
  }
  return N2 && l4 ? a2[1] = (o2 + s5) / 2 : a2[0] = a2[1] = NaN, a2;
}
function x2(t4, n2) {
...
function M(t4, e2, n2) {
  ...
  const a2 = t4[0], o2 = t4[1], s5 = t4[2], c2 = t4[3];
  if (i2 & r) return 0;
  if (!(i2 | r)) return 4;
  const N2 = (i2 ? 1 : 0) | (r ? 2 : 0);
  do {
    const N3 = n2[0] - e2[0], l4 = n2[1] - e2[1];
    if (N3 > l4) i2 & P ? (i2 & m2 ? (e2[1] += l4 * (a2 - e2[0]) / N3, e2[0] = a2) : (e2[1] += l4 * (s5 - e2[0]) / N3, e2[0] = s5), i2 = b(e2, t4)) : r & P ? (r & m2 ? (n2[1] += l4 * (a2 - n2[0]) / N3, n2[0] = a2) : (n2[1] += l4 * (s5 - n2[0]) / N3, n2[0] = s5), r = b(n2, t4)) : i2 ? (i2 & d2 ? (e2[0] += N3 * (o2 - e2[1]) / l4, e2[1] = o2) : (e2[0] += N3 * (c2 - e2[1]) / l4, e2[1] = c2), i2 = b(e2, t4)) : (r & d2 ? (n2[0] += N3 * (o2 - n2[1]) / l4, n2[1] = o2) : (n2[0] += N3 * (c2 - n2[1]) / l4, n2[1] = c2), r = b(n2, t4));
    else if (i2 & y2 ? (i2 & d2 ? (e2[0] += N3 * (o2 - e2[1]) / l4, e2[1] = o2) : (e2[0] += N3 * (c2 - e2[1]) / l4, e2[1] = c2), i2 = b(e2, t4)) : r & y2 ? (r & d2 ? (n2[0] += N3 * (o2 - n2[1]) / l4, n2[1] = o2) : (n2[0] += N3 * (c2 - n2[1]) / l4, n2[1] = c2), r = b(n2, t4)) : i2 ? (i2 & m2 ? (e2[1] += l4 * (a2 - e2[0]) / N3, e2[0] = a2) : (e2[1] += l4 * (s5 - e2[0]) / N3, e2[0] = s5), i2 = b(e2, t4)) : (r & m2 ? (n2[1] += l4 * (a2 - n2[0]) / N3, n2[0] = a2) : (n2[1] += l4 * (s5 - n2[0]) / N3, n2[0] = s5), r = b(n2, t4)), i2 & r) return 0;
  } while (i2 | r);
  return N2;
}
function b(t4, e2) {
...
//# sourceMappingURL=chunk-FGKJNA36.js.map
