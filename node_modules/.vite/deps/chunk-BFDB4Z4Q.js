...
var a3 = class extends b {
  ...
  add(e2, t3) {
    ...
    if (!(r2 == null ? void 0 : r2.length)) return;
  ...
  remove(e2) {
    ...
    (t3 == null ? void 0 : t3.length) && t3.forEach((e3) => e3 && this._removeHighlight(this.getKeyForFeature(e3)));
  }
  removeByKey(e2) {
...
var M = class extends l {
  ...
  initialize() {
    t(this.view, this.layer), this._highlightHelper = new l4({ view: this.view }), this.refresh(), this.addHandles([l2(() => this.graphics.length, () => this.refresh()), f(() => {
      ...
      this.addHandles([this.view.on("immediate-click", (i2) => this._clickHandler(i2), y.TOOL), this.view.on("double-click", (i2) => this._doubleClickHandler(i2), y.TOOL), this.view.on("pointer-down", (i2) => this._pointerDownHandler(i2), y.TOOL), this.view.on("pointer-move", (i2) => this._pointerMoveHandler(i2), y.TOOL), this.view.on("pointer-up", (i2) => this._pointerUpHandler(i2), y.TOOL), this.view.on("drag", (i2) => this._dragHandler(i2), y.TOOL), this.view.on("key-down", (i2) => this._keyDownHandler(i2), y.TOOL)]);
  ...
  destroy() {
    var _a;
    this._removeIndicators(), (_a = this.view.map) == null ? void 0 : _a.remove(this.layer), u(this._defaultLayer), this.reset();
  }
  get state() {
  ...
  _pointerMoveHandler(i2) {
    ...
    if (t3) {
      ...
      if (this._hoverGraphic) {
        ...
        this._hoverGraphic = null, this.emit("graphic-pointer-out", h4), (_b = (_a = this.callbacks).onGraphicPointerOut) == null ? void 0 : _b.call(_a, h4);
      }
      const r2 = this.graphics.indexOf(t3), h3 = new r(t3, r2, e2, s2, i2);
      return this._hoverGraphic = t3, this.emit("graphic-pointer-over", h3), void ((_d = (_c = this.callbacks).onGraphicPointerOver) == null ? void 0 : _d.call(_c, h3));
    }
    if (this._hoverGraphic) {
  ...
  _dragHandler(i2) {
    ...
    const { action: t3, x: s2, y: r2 } = i2, h3 = this.graphics.indexOf(this._activeGraphic), o = this._dragEvent ? s2 - this._dragEvent.x : 0, a5 = this._dragEvent ? r2 - this._dragEvent.y : 0, n4 = s2 - i2.origin.x, c4 = r2 - i2.origin.y, l5 = "start" === t3 ? this._activeGraphic.geometry : this._initialDragGeometry, p = x(l5, n4, c4, this.view);
    if (this._activeGraphic.geometry = p, this.enableMoveAllGraphics && this.graphics.forEach((i3) => {
      ...
      this._dragEvent = null, this._activeGraphic = null, this._setUpIndicators(), this.emit("graphic-move-stop", t4), (_g = (_f = this.callbacks).onGraphicMoveStop) == null ? void 0 : _g.call(_f, t4), t4.defaultPrevented && (this.graphics[h3].set("geometry", this._initialDragGeometry), this._setUpIndicators()), this._initialDragGeometry = null;
    }
  }
  _keyDownHandler(i2) {
  ...
  _findTargetGraphic(i2) {
    const t3 = this.graphics;
    this._syncLayerViews();
    const e2 = this._layerViews.flatMap((t4) => "graphicsViews" in t4 ? Array.from(t4.graphicsViews(), (t5) => t5.hitTest(this.view, i2)).flat() : t4.graphicsView.hitTest(this.view, i2)).filter((i3) => t3.includes(i3)).sort((i3, e3) => t3.indexOf(i3) - t3.indexOf(e3));
    return e2.length ? e2[0] : null;
  }
  _syncLayerViews() {
    this._layerViews = [];
  ...
  _setUpIndicators() {
    if (this._removeIndicators(), this.indicatorsEnabled) {
      for (const i2 of this.graphics) {
        const t3 = i2.clone();
        t3.symbol = U(i2), this._indicators.push(t3), this.addHandles(l2(() => i2.symbol, () => this._setUpIndicators()), E);
      }
      this.layer.addMany(this._indicators);
    }
  }
  _removeIndicators() {
    this.removeHandles(E), this._indicators.length && (this.layer.removeMany(this._indicators), this._indicators.forEach((i2) => i2.destroy()), this._indicators = []);
  }
};
function U(i2) {
  const t3 = 12;
  ...
  switch (i2.symbol.type) {
    case "cim":
      return new u2({ style: "circle", size: t3, color: [0, 0, 0, 0], outline: { color: [255, 127, 0, 1], width: 1 } });
    case "picture-marker": {
      const { xoffset: t4, yoffset: e2, height: s2, width: r2 } = i2.symbol, h3 = s2 === r2 ? r2 : Math.max(s2, r2);
      return new u2({ xoffset: t4, yoffset: e2, size: h3, style: "square", color: [0, 0, 0, 0], outline: { color: [255, 127, 0, 1], width: 1 } });
    }
    case "simple-marker": {
      const { xoffset: t4, yoffset: e2, size: s2, style: r2 } = i2.symbol;
      return new u2({ xoffset: t4, yoffset: e2, style: "circle" === r2 ? "circle" : "square", size: s2 + 2, color: [0, 0, 0, 0], outline: { color: [255, 127, 0, 1], width: 1 } });
    }
    case "simple-fill":
    ...
    case "text": {
      ...
      return new u2({ xoffset: e2, yoffset: s2, size: t3, color: [0, 0, 0, 0], outline: { color: [255, 127, 0, 1], width: 1 } });
    }
    default:
      return null;
  }
...
//# sourceMappingURL=chunk-BFDB4Z4Q.js.map
