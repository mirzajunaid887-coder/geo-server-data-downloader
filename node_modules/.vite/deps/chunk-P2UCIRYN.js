...
function i2(n2, t2) {
  ...
  if (n2.ymin <= t2.ymin) {
    if (o3 = t2.ymin, n2.ymax < o3) return false;
  } else if (o3 = n2.ymin, t2.ymax < o3) return false;
  if (e2 && t2.hasZ) {
    if (n2.zmin <= t2.zmin) {
      if (i3 = t2.zmin, n2.zmax < i3) return false;
    } else if (i3 = n2.zmin, t2.zmax < i3) return false;
  }
  return true;
}
function f2(t2, e2) {
  const { points: o3, hasZ: i3 } = e2, f4 = i3 ? e : i;
  for (const n2 of o3) if (f4(t2, n2)) return true;
  return false;
}
var s2 = [0, 0];
var c3 = [0, 0];
...
var m2 = [0, 0];
var a2 = [s2, c3, u2, m2];
var l2 = [[u2, s2], [s2, c3], [c3, m2], [m2, u2]];
function x2(n2, t2) {
  return h(n2, t2.rings);
}
function h(t2, r3) {
  s2[0] = t2.xmin, s2[1] = t2.ymax, c3[0] = t2.xmax, c3[1] = t2.ymax, u2[0] = t2.xmin, u2[1] = t2.ymin, m2[0] = t2.xmax, m2[1] = t2.ymin;
  ...
  for (const e2 of r3) {
    ...
    for (let o3 = 1; o3 < e2.length; o3++) {
      const i3 = e2[o3];
      if (i(t2, i3) || g(r4, i3, l2)) return true;
      r4 = i3;
    }
  }
  return false;
...
function y(t2, e2) {
  ...
  for (const o3 of r3) {
    ...
    for (let r4 = 1; r4 < o3.length; r4++) {
      const i3 = o3[r4];
      if (i(t2, i3) || g(e3, i3, l2)) return true;
      e3 = i3;
    }
  }
  return false;
...
function g(n2, t2, e2) {
  for (let r3 = 0; r3 < e2.length; r3++) if (p(n2, t2, e2[r3][0], e2[r3][1])) return true;
  return false;
}
function p(n2, t2, e2, r3, o3) {
  const [i3, f4] = n2, [s3, c4] = t2, [u4, m3] = e2, [a3, l3] = r3, x3 = a3 - u4, h2 = i3 - u4, y2 = s3 - i3, g2 = l3 - m3, p2 = f4 - m3, z3 = c4 - f4, G3 = g2 * y2 - x3 * z3;
  if (0 === G3) return false;
  const M4 = (x3 * p2 - g2 * h2) / G3, P2 = (y2 * p2 - z3 * h2) / G3;
  return M4 >= 0 && M4 <= 1 && P2 >= 0 && P2 <= 1 && (o3 && (o3[0] = i3 + M4 * (s3 - i3), o3[1] = f4 + M4 * (c4 - f4)), true);
}
function z(n2, t2, e2) {
  for (const r3 of t2) {
    const t3 = r3.length;
    for (let o3 = 1; o3 < t3; o3++) {
      const t4 = r3[o3 - 1], i3 = r3[o3];
      let f4 = (i3[0] - t4[0]) * (i3[0] - t4[0]) + (i3[1] - t4[1]) * (i3[1] - t4[1]);
      if (0 === f4) continue;
      f4 = Math.sqrt(f4);
      const s3 = ((i3[0] - t4[0]) * (n2[1] - t4[1]) - (i3[1] - t4[1]) * (n2[0] - t4[0])) / f4;
      if (Math.abs(s3) < e2) {
        const r4 = ((i3[0] - t4[0]) * (n2[0] - t4[0]) + (i3[1] - t4[1]) * (n2[1] - t4[1])) / f4;
        if (r4 > -e2 && r4 < f4 + e2) return true;
      }
    }
  }
  return false;
...
function G2(n2) {
  const t2 = 1e-6, e2 = n2[0], r3 = n2[n2.length - 1];
  return Math.abs(e2[0] - r3[0]) <= t2 && Math.abs(e2[1] - r3[1]) <= t2;
}
function M2(n2, t2) {
  let e2 = 0;
  for (const r3 of t2) {
    ...
    for (let o3 = 1; o3 < t3; o3++) {
      const t4 = r3[o3 - 1], i3 = r3[o3];
      if (t4[1] > n2[1] == i3[1] > n2[1]) continue;
      (i3[0] - t4[0]) * (n2[1] - t4[1]) - (i3[1] - t4[1]) * (n2[0] - t4[0]) > 0 ? e2++ : e2--;
    }
  }
  return 0 !== e2;
}
function P(n2, t2) {
  n2.length > 1 && !G2(n2) && (n2 = [...n2, n2[0]]);
  for (let e2 = 0; e2 < t2.length; e2++) {
    const r3 = t2[e2], o3 = r3.length;
    for (const t3 of r3) if (M2(t3, [n2])) return true;
    if (G2(r3)) {
      for (const e3 of n2) if (M2(e3, t2)) return true;
    }
    for (let t3 = 1; t3 < o3; t3++) {
      const e3 = r3[t3 - 1], o4 = r3[t3];
      for (let t4 = 0; t4 < n2.length; t4++) {
        if (p(e3, o4, n2[t4], n2[(t4 + 1) % n2.length])) return true;
      }
    }
  }
  return false;
...
var z2 = u3 = class extends s {
  ...
  _shiftCM(t2 = G(this.spatialReference)) {
    ...
    if (s3) {
      ...
      this.xmin -= s3.x, this.xmax -= s3.x, e2.isWebMercator || (n2.x = M3(n2.x, t2).x), this.spatialReference = new S(r((e2.isWGS84 ? t2.altTemplate : null) ?? t2.wkTemplate, { Central_Meridian: n2.x }));
    }
    return this;
  }
  _getCM(t2) {
...
//# sourceMappingURL=chunk-P2UCIRYN.js.map
