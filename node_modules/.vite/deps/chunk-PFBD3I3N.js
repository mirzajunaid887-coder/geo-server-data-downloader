...
function s(e2) {
  if (null != e2 && (e2 == null ? void 0 : e2.aborted)) throw new r4("Cancelled");
}
...
function l2(e2, r5) {
  return g(e2 == null ? void 0 : e2.parseTree, r5, e2 == null ? void 0 : e2.parameters);
}
  ...
  switch (t3.type) {
    ...
    case "function":
      switch (t3.name.toLowerCase()) {
        case "cast":
          switch (((_b = (_a2 = t3.args) == null ? void 0 : _a2.value[1]) == null ? void 0 : _b.value.type) ?? "") {
            case "integer":
            case "smallint":
              return "integer";
            case "real":
            ...
            case "date":
            case "timestamp":
              return true === ((_e = (_d = (_c = t3.args) == null ? void 0 : _c.value[1]) == null ? void 0 : _d.value) == null ? void 0 : _e.withtimezone) ? "timestamp-offset" : "date";
            case "time":
              return "time-only";
            case "varchar":
              return "string";
...
async function k(n, e2, r5) {
  ...
  for await (const u3 of a5) {
    if (l4.ticker += u3.length, l4.ticker >= 100 && (l4.ticker = 0, await new Promise((t3) => {
      setTimeout(t3, 0);
    })), r5 == null ? void 0 : r5.aborted) throw new r4("Cancelled");
    for (const t3 of u3) {
      const n2 = e2 == null ? void 0 : e2.calculateValue(t3);
...
async function S4(n, e2, r5 = 1e3, a5 = null) {
  ...
  for await (const h3 of s3) {
    if (f3.ticker += h3.length, f3.ticker >= 100 && (f3.ticker = 0, await new Promise((t3) => {
      setTimeout(t3, 0);
    })), a5 == null ? void 0 : a5.aborted) throw new r4("Cancelled");
    for (const t3 of h3) {
...
var i2 = class {
  ...
  async _nextBatch() {
    ...
    const t3 = await (this._features ?? (this._features = this._parent.queryAll(this._abortSignal)));
    let r5;
    do {
...
function I3(e2) {
  switch (e2) {
    case "esriSpatialRelIntersects":
      return { spatialRelationship: "intersects", relationParameter: null };
    case "esriSpatialRelContains":
      return { spatialRelationship: "contains", relationParameter: null };
    case "esriSpatialRelOverlaps":
      return { spatialRelationship: "overlaps", relationParameter: null };
    case "esriSpatialRelWithin":
      return { spatialRelationship: "within", relationParameter: null };
    case "esriSpatialRelTouches":
      return { spatialRelationship: "touches", relationParameter: null };
    case "esriSpatialRelCrosses":
      return { spatialRelationship: "crosses", relationParameter: null };
    case "esriSpatialRelEnvelopeIntersects":
      return { spatialRelationship: "envelope-intersects", relationParameter: null };
    default:
      if (e2.includes("esriSpatialRelRelation")) return { spatialRelationship: "relation", relationParameter: e2.split(":")[1] };
      throw new r4("NeverReach");
  }
}
function w3(e2, t3) {
  var _a2;
  for (const a5 of e2) null != a5.geometry && ((_a2 = a5.geometry).spatialReference ?? (_a2.spatialReference = t3));
}
var S5 = Object.freeze({ ordered: true, filterApplied: true, spatialFilterApplied: true, features: Object.freeze({ next: () => Promise.resolve({ done: true, value: void 0 }), [Symbol.asyncIterator]() {
  return this;
} }) });
var R2 = class {
  constructor() {
  ...
  async loadImpl() {
    var _a2;
    return await ((_a2 = this._parent) == null ? void 0 : _a2.load()), this._initialiseFeatureSet(), this;
  }
  ...
  async queryAll(e2) {
    null == this._allFeatures && (this._allFeatures = this._queryAll().then((e3) => e(e3)));
    const a5 = (await this._allFeatures)();
    return null == e2 || e2 === t ? a5 : async function* () {
      for await (const t3 of a5) s(e2), yield t3;
    }();
  }
  async databaseType() {
  ...
  async first(t3) {
    if (this._hasCachedFeatures) return this.iterator(t3).next();
    const a5 = await this.query({ limit: 1, abortSignal: t3 });
    for await (const s3 of a5.features) if (s3.length > 0) return j.fromJSON(s3[0]);
    return null;
  }
  async isEmpty(e2) {
  ...
  async count(e2) {
    return await this.load(), this.calculateStatistic("count", M.create("1", { fieldsIndex: this.getFieldsIndex(), timeZone: this.dateFieldsTimeZoneDefaultUTC }), -1, e2 ?? t);
  }
  castToText(e2 = false) {
    ...
    if ("featureset" === e2) {
      ...
      for await (const a5 of await this.queryAll(t3)) for (const t4 of a5) {
        ...
        if (null !== a6.geometry && a6.geometry.spatialReference) {
          const { spatialReference: e4, ...t5 } = a6.geometry;
          a6.geometry = t5;
        }
        e3.push(a6);
      }
      return JSON.stringify({ ...this.schema(), features: e3, spatialReference: this.spatialReference.toJSON() });
    }
  ...
  async castAsJsonAsync(e2 = null, t3 = null) {
    ...
    for await (const s3 of await this.queryAll(e2)) for (const e3 of s3) {
      const s4 = { geometry: e3.geometry ? true === (t3 == null ? void 0 : t3.keepGeometryType) ? u(e3.geometry) : e3.geometry : null, attributes: e3.attributes };
      if (null !== s4.geometry && s4.geometry.spatialReference && true !== (t3 == null ? void 0 : t3.keepGeometryType)) {
        delete s4.geometry.spatialReference;
        const { spatialReference: e4, ...t4 } = s4.geometry;
        s4.geometry = t4;
      }
      a5.push(s4);
    }
    return { ...this.schema(), features: a5, spatialReference: true === (t3 == null ? void 0 : t3.keepGeometryType) ? this.spatialReference : (_a2 = this.spatialReference) == null ? void 0 : _a2.toJSON() };
  }
  fieldTimeZone(e2) {
...
//# sourceMappingURL=chunk-PFBD3I3N.js.map
