...
async function S2(n3, o, a = {}, s = 5) {
  const { links: l } = n3, c = A(l);
  if (!c) throw new r("ogc-feature-layer:missing-items-page", "Missing items url");
  const { apiKey: u2, customParameters: f2, signal: d2 } = a, g2 = _(c.href, n3.landingPage.url), w = { limit: s, ...f2, token: u2 }, T3 = Bt(g2, w), k2 = { accept: "application/geo+json" }, { data: x2 } = await f(T3, { signal: d2, headers: k2 }), S3 = E(T3, s, x2.links) ?? I2;
  M(x2);
  ...
  for (const e2 of P2) {
    if (e2.name ?? (e2.name = e2.alias), e2.alias ?? (e2.alias = e2.name), "esriFieldTypeOID" !== e2.type && "esriFieldTypeGlobalID" !== e2.type && (e2.editable ?? (e2.editable = true), e2.nullable ?? (e2.nullable = true)), !e2.name) throw new r("ogc-feature-layer:invalid-field-name", "field name is missing", { field: e2 });
  ...
  return { drawingInfo: C2 ? u(C2) : null, extent: U(n3), geometryType: C2, fields: P2, hasZ: !!q2, objectIdField: N2, paginationParameter: S3, timeInfo: R2, featureCount: v2 };
}
async function v(n3, r2 = {}) {
  const { links: o, url: a } = n3, s = L(o);
  if (!s) throw new r("ogc-feature-layer:missing-collections-page", "Missing collections url");
  const { apiKey: l, customParameters: c, signal: u2 } = r2, f2 = _(s.href, a), { data: d2 } = await f(f2, { signal: u2, headers: { accept: "application/json" }, query: { ...c, token: l } });
  for (const e2 of d2.collections) e2.landingPage = n3;
  return d2;
}
async function O2(n3, r2 = {}) {
  const { links: o, url: a } = n3, s = K(o);
  if (!s) throw new r("ogc-feature-layer:missing-conformance-page", "Missing conformance url");
  const { apiKey: l, customParameters: c, signal: u2 } = r2, f2 = _(s.href, a), { data: d2 } = await f(f2, { signal: u2, headers: { accept: "application/json" }, query: { ...c, token: l } });
  return d2;
}
async function P(t, n3 = {}) {
  const { apiKey: i2, customParameters: r2, signal: o } = n3, { data: a } = await f(t, { signal: o, headers: { accept: "application/json" }, query: { ...r2, token: i2 } });
  return a.url = t, a;
}
async function q(t, n3 = {}) {
  const { links: r2, url: o } = t, a = J2(r2, "service-desc", "application/vnd.oai.openapi+json;version=3.0");
  if (!a) return F().warn("ogc-feature-layer:missing-openapi-page", "The OGC API-Features server does not have an OpenAPI page."), null;
  const { apiKey: s, customParameters: l, signal: c } = n3, u2 = _(a.href, o), { data: f2 } = await f(u2, { signal: c, headers: { accept: "application/vnd.oai.openapi+json;version=3.0" }, query: { ...l, token: s } });
  return f2;
}
function C(e2) {
...
async function R(n3, r2, o) {
  const { collection: { links: c, landingPage: { url: p } }, layerDefinition: m, maxRecordCount: y2, queryParameters: { apiKey: b, customParameters: j }, spatialReference: F2, supportedCrs: I3 } = n3, T3 = A(c);
  ...
  const P2 = S.fromJSON(F2), q2 = r2.outSpatialReference ?? P2, C2 = q2.isWGS84 ? null : $(q2, I3), N2 = Z(k2, I3), R2 = M2(v2), W3 = D(O3), G2 = x2 ?? (null == S3 ? y2 : 10), K2 = 0 === S3 ? void 0 : S3, { fields: L2, geometryType: U2, hasZ: J3, objectIdField: E2, paginationParameter: _3 } = m, B = _(T3.href, p), { data: Q } = await f(B, { ...o, query: { ...j, ...N2, crs: C2, datetime: R2, query: W3, limit: G2, [_3]: K2, token: b }, headers: { accept: "application/geo+json" } }), V = O(Q, { geometryType: U2, hasZ: J3, objectIdField: E2 }), H2 = V.length === G2 && !!z(Q.links ?? []), X = new _2(L2);
  for (const e2 of V) {
    const t = {};
  ...
  if (!C2 && q2.isWebMercator) {
    for (const e2 of V) if (null != e2.geometry && null != U2) {
      const t = J(e2.geometry, U2, J3, false);
  ...
  return ee.exceededTransferLimit = H2, ee.features = V, ee.fields = L2, ee.geometryType = U2, ee.hasZ = J3, ee.spatialReference = Y, ee;
}
function W2(e2) {
...
function K(e2) {
  return J2(e2, "conformance", "application/json") ?? J2(e2, "http://www.opengis.net/def/rel/ogc/1.0/conformance", "application/json");
}
function L(e2) {
  return J2(e2, "data", "application/json") ?? J2(e2, "http://www.opengis.net/def/rel/ogc/1.0/data", "application/json");
}
function U(e2) {
  var _a;
...
function A(e2) {
  return J2(e2, "items", "application/geo+json", "http://www.opengis.net/def/profile/ogc/0/rfc7946") ?? J2(e2, "http://www.opengis.net/def/rel/ogc/1.0/items", "application/geo+json", "http://www.opengis.net/def/profile/ogc/0/rfc7946");
}
function J2(e2, t, n3, i2) {
  return i2 ? e2.find(({ rel: e3, type: r2, profile: o }) => e3 === t && r2 === n3 && (o == null ? void 0 : o.includes(i2))) ?? e2.find(({ rel: e3, type: i3, profile: r2 }) => e3 === t && i3 === n3 && !r2) : e2.find(({ rel: e3, type: i3 }) => e3 === t && i3 === n3) ?? e2.find(({ rel: e3, type: n4 }) => e3 === t && !n4);
}
function z(e2) {
  return J2(e2, "next", "application/geo+json");
}
function E(e2, t, n3) {
  var _a;
  if (!n3) return;
  const i2 = z(n3), r2 = (_a = I(i2 == null ? void 0 : i2.href)) == null ? void 0 : _a.query;
  if (!r2) return;
  const a = I(e2).query, s = Object.keys(a ?? {}), l = Object.entries(r2).filter(([e3]) => !s.includes(e3)).find(([e3, n4]) => T2.has(e3.toLowerCase()) && Number.parseInt(n4, 10) === t), c = l == null ? void 0 : l[0];
  return c;
}
...
//# sourceMappingURL=chunk-YHYYTYJA.js.map
