import {
  P,
  Q,
  X,
  k,
  q,
  v
} from "./chunk-HDPYXFMH.js";
import {
  S,
  b,
  k as k2
} from "./chunk-TEC3EFEB.js";
import {
  s
} from "./chunk-3U3RVUQO.js";
import {
  f,
  r as r2
} from "./chunk-LM6JHL3H.js";
import {
  r
} from "./chunk-I5UAHQ4J.js";
import {
  z
} from "./chunk-5BY2LZNH.js";
import {
  a,
  e,
  u,
  x
} from "./chunk-FTY3ENIR.js";
import {
  c,
  h
} from "./chunk-IUMZVY6E.js";
import {
  n
} from "./chunk-WT4NB7ZZ.js";

// node_modules/@arcgis/core/geometry/support/clipRay.js
function i(r3) {
  return r3 ? { ray: b(r3.ray), c0: r3.c0, c1: r3.c1 } : { ray: b(), c0: 0, c1: Number.MAX_VALUE };
}
function m(r3, c2 = i()) {
  return k2(r3, c2.ray), c2.c0 = 0, c2.c1 = Number.MAX_VALUE, c2;
}
function s2(r3, n2, t = i()) {
  const o = a(r3.vector);
  return S(r3.origin, n2, t.ray), t.c0 = 0, t.c1 = o, t;
}
function p(r3, c2) {
  return g(r3, r3.c0, c2);
}
function b2(r3, c2) {
  return g(r3, r3.c1, c2);
}
function g(r3, c2, o) {
  return e(o, r3.ray.origin, x(o, r3.ray.direction, c2));
}
var j = new s(() => i());

// node_modules/@arcgis/core/geometry/support/frustum.js
function v2(r3) {
  return r3 ? [v(r3[0]), v(r3[1]), v(r3[2]), v(r3[3]), v(r3[4]), v(r3[5])] : [v(), v(), v(), v(), v(), v()];
}
function h2() {
  return [n(), n(), n(), n(), n(), n(), n(), n()];
}
function k3(r3, t) {
  for (let e2 = 0; e2 < B; e2++) q(r3[e2], t[e2]);
  return r3;
}
function y(r3, n2, i2, f2 = F) {
  const m2 = c(f.get(), n2, r3);
  h(m2, m2);
  for (let t = 0; t < C; ++t) {
    const r4 = z(r2.get(), D[t], m2);
    u(f2[t], r4[0] / r4[3], r4[1] / r4[3], r4[2] / r4[3]);
  }
  S2(i2, f2);
}
function S2(r3, t) {
  k(t[4], t[0], t[3], r3[0]), k(t[1], t[5], t[6], r3[1]), k(t[4], t[5], t[1], r3[2]), k(t[3], t[2], t[6], r3[3]), k(t[0], t[1], t[2], r3[4]), k(t[5], t[4], t[7], r3[5]);
}
function d(r3, t) {
  for (let e2 = 0; e2 < B; e2++) {
    const o = r3[e2];
    if (o[0] * t.center[0] + o[1] * t.center[1] + o[2] * t.center[2] + o[3] >= t.radius) return false;
  }
  return true;
}
function w(r3, t) {
  return z2(r3, m(t, E.get()));
}
function O(r3, t) {
  for (let e2 = 0; e2 < B; e2++) {
    const o = r3[e2];
    if (!Q(o, t)) return false;
  }
  return true;
}
function R(r3, t, e2) {
  return z2(r3, s2(t, e2, E.get()));
}
function q2(r3, t) {
  for (let e2 = 0; e2 < B; e2++) {
    if (X(r3[e2], t) > 0) return false;
  }
  return true;
}
function z2(r3, t) {
  for (let e2 = 0; e2 < B; e2++) if (!P(r3[e2], t)) return false;
  return true;
}
var A = { bottom: [5, 1, 0, 4], near: [0, 1, 2, 3], far: [5, 4, 7, 6], right: [1, 5, 6, 2], left: [4, 0, 3, 7], top: [7, 3, 2, 6] };
var B = 6;
var C = 8;
var D = [r(-1, -1, -1, 1), r(1, -1, -1, 1), r(1, 1, -1, 1), r(-1, 1, -1, 1), r(-1, -1, 1, 1), r(1, -1, 1, 1), r(1, 1, 1, 1), r(-1, 1, 1, 1)];
var E = new s(i);
var F = h2();

export {
  i,
  m,
  p,
  b2 as b,
  v2 as v,
  h2 as h,
  k3 as k,
  y,
  S2 as S,
  d,
  w,
  O,
  R,
  q2 as q,
  A
};
//# sourceMappingURL=chunk-K4QIA3R6.js.map
