...
function o2(t2) {
  ...
  let d3 = h3 / (8 * u4), p = h3 / (8 * f3);
  if (r3 && h3 > 1e-3 && (d3 /= n3, p /= n3), "adjusted" === l4) if (r3) {
    const t3 = u4 * n3, e3 = f3 * n3;
    d3 = (h3 + t3 ** o3 * a6) / (8 * t3), p = (h3 + e3 ** o3 * a6) / (8 * e3);
  } else d3 = (h3 + u4 ** o3 * a6) / (8 * u4), p = (h3 + f3 ** o3 * a6) / (8 * f3);
  let x = (90 - e2) * Math.PI / 180, w = Math.cos(x), y = (360 - i + 90) * Math.PI / 180, m2 = Math.sin(x) * Math.cos(y), M = Math.sin(x) * Math.sin(y);
  ...
  if ("multi-directional" === s2) for (let n4 = 0; n4 < P; n4++) e2 = A[n4], i = z2[n4], x = (90 - e2) * Math.PI / 180, w = Math.cos(x), y = (360 - i + 90) * Math.PI / 180, m2 = Math.sin(x) * Math.cos(y), M = Math.sin(x) * Math.sin(y), S[n4] = w, C[n4] = m2, T2[n4] = M;
  else S.fill(w), C.fill(m2), T2.fill(M);
  return { resolution: c5, factor: [d3, p], sinZcosA: m2, sinZsinA: M, cosZ: w, sinZcosAs: [...C], sinZsinAs: [...T2], cosZs: [...S], weights: [...k], hillshadeType: ["traditional", "multi-directional"].indexOf(s2) };
}
function a5(t2, e2) {
  ...
  if (e2) {
    ...
    d3 = 200 * e2.zFactor / (i2 * i2), f3 = new Float32Array(o3);
  }
  const p = e2 == null ? void 0 : e2.curvatureType;
  let x, w, y, m2, M, z2, A, g;
  for (let F = s; F < n4 - s; F++) {
    const t3 = F * i;
    for (let e3 = s; e3 < i - s; e3++) {
      const s2 = t3 + e3;
      if (0 !== (r3 == null ? void 0 : r3[s2])) {
        if (r3) {
          if (r3[s2 - i - 1] + r3[s2 - i] + r3[s2 - i + 1] + r3[s2 - 1] + r3[s2 + 1] + r3[s2 + i - 1] + r3[s2 + i] + r3[s2 + i + 1] < 7) {
            a6[s2] = 0;
            continue;
          }
          x = r3[s2 - i - 1] ? c5[s2 - i - 1] : c5[s2], w = r3[s2 - i] ? c5[s2 - i] : c5[s2], y = r3[s2 - i + 1] ? c5[s2 - i + 1] : c5[s2], m2 = r3[s2 - 1] ? c5[s2 - 1] : c5[s2], M = r3[s2 + 1] ? c5[s2 + 1] : c5[s2], z2 = r3[s2 + i - 1] ? c5[s2 + i - 1] : c5[s2], A = r3[s2 + i] ? c5[s2 + i] : c5[s2], g = r3[s2 + i + 1] ? c5[s2 + i + 1] : c5[s2];
        } else x = c5[s2 - i - 1], w = c5[s2 - i], y = c5[s2 - i + 1], m2 = c5[s2 - 1], M = c5[s2 + 1], z2 = c5[s2 + i - 1], A = c5[s2 + i], g = c5[s2 + i + 1];
        if (h3[s2] = y + M + M + g - (x + m2 + m2 + z2), u4[s2] = z2 + A + A + g - (x + w + w + y), null != f3) {
          const t4 = c5[s2], e4 = 0.5 * (m2 + M) - t4, i2 = 0.5 * (w + A) - t4;
          if ("standard" === p) f3[s2] = -d3 * (e4 + i2);
          else {
            const t5 = (-x + y + z2 - g) / 4, n5 = (-m2 + M) / 2, o4 = (w - A) / 2, a7 = n5 * n5, l5 = o4 * o4, r4 = a7 + l5;
            r4 && (f3[s2] = "profile" === p ? d3 * (e4 * a7 + i2 * l5 + t5 * n5 * o4) / r4 : -d3 * (e4 * l5 + i2 * a7 - t5 * n5 * o4) / r4);
          }
        }
      }
    }
...
function r2(n4, r3) {
  if (!o(n4)) return n4;
  const { factor: c5, sinZcosA: h3, sinZsinA: u4, cosZ: f3, sinZcosAs: d3, sinZsinAs: p, cosZs: x, weights: w } = o2(r3), [y, m2] = c5, M = "traditional" === r3.hillshadeType, { width: z2, height: A } = n4, g = new Uint8Array(z2 * A), { dzxs: F, dzys: k, outMask: P } = a5(n4);
      ...
      if (!P || P[i]) {
        ...
        if (M) {
          ...
          for (let o3 = 0; o3 < i2; o3++) {
            let i3 = 255 * (x[o3] + p[o3] * e3 - d3[o3] * t4) / s2;
...
function c4(e2, i, s2, n4) {
  if (!o(e2) || !o(i)) return;
  const { min: o3, max: a6 } = n4, l4 = e2.pixels[0], { pixels: r3, mask: c5 } = i, h3 = r3[0], u4 = 255.00001 / (a6 - o3), f3 = new Uint8ClampedArray(h3.length), d3 = new Uint8ClampedArray(h3.length), p = new Uint8ClampedArray(h3.length), x = s2.length - 1;
  for (let t2 = 0; t2 < h3.length; t2++) {
    if (0 === (c5 == null ? void 0 : c5[t2])) continue;
    const e3 = Math.floor((h3[t2] - o3) * u4), [i2, n5] = s2[e3 < 0 ? 0 : e3 > x ? x : e3], a7 = l4[t2], r4 = a7 * n5, w = r4 * (1 - Math.abs(i2 % 2 - 1)), y = a7 - r4;
    switch (Math.floor(i2)) {
      case 0:
        f3[t2] = r4 + y, d3[t2] = w + y, p[t2] = y;
        break;
      case 1:
        f3[t2] = w + y, d3[t2] = r4 + y, p[t2] = y;
        break;
      case 2:
        f3[t2] = y, d3[t2] = r4 + y, p[t2] = w + y;
        break;
      case 3:
        f3[t2] = y, d3[t2] = w + y, p[t2] = r4 + y;
        break;
      case 4:
        f3[t2] = w + y, d3[t2] = y, p[t2] = r4 + y;
        break;
      case 5:
      case 6:
        f3[t2] = r4 + y, d3[t2] = y, p[t2] = w + y;
    }
  }
  e2.pixels = [f3, d3, p], e2.updateStatistics();
}
function h2(i, o3) {
  if (!o(i)) return i;
  const r3 = o3.zFactor, c5 = o3.pixelSizePower ?? 1, h3 = o3.pixelSizeFactor ?? 1, u4 = o3.slopeType, f3 = o3.isGCS, { x: d3, y: p } = o3.resolution;
  let x = r3 / (8 * d3), w = r3 / (8 * p);
  f3 && Math.abs(r3 - 1) < 1e-4 && (x /= n3, w /= n3), "adjusted" === u4 && (x = (r3 + d3 ** c5 * h3) / (8 * d3), w = (r3 + p ** c5 * h3) / (8 * p));
  ...
  for (let t2 = s; t2 < A - s; t2++) {
    ...
    for (let t3 = s; t3 < z2 - s; t3++) {
      const i2 = e2 + t3;
      if (!M || M[i2]) {
        const s2 = y[i2] * x, n4 = m2[i2] * w, o4 = Math.sqrt(s2 * s2 + n4 * n4);
...
function u3(n4, o3 = {}) {
  if (!o(n4)) return n4;
  const { resolution: r3 } = o3, c5 = r3 ? 1 / r3.x : 1, h3 = r3 ? 1 / r3.y : 1, { dzxs: u4, dzys: f3, outMask: d3 } = a5(n4), { width: p, height: x } = n4, w = new Float32Array(p * x);
  for (let t2 = s; t2 < x - s; t2++) {
    const e2 = t2 * p;
  ...
  l3(w, p, x);
  return new c2({ width: p, height: x, pixels: [w], mask: d3, pixelType: "f32", validPixelCount: n4.validPixelCount, statistics: [new l(-1, 360)] });
}
function f2(i, s2) {
  if (!o(i)) return i;
...
var O = class extends n2 {
  ...
  symbolize(e2) {
    let t2 = e2 == null ? void 0 : e2.pixelBlock;
    if (!T(t2)) return t2;
  ...
  simpleStretch(e2, t2) {
    if (!T(e2)) return e2;
    try {
      return e2.pixels.length > 3 && (e2 = e2.extractBands([0, 1, 2])), z(e2, { ...t2, isRenderer: true });
    } catch (s2) {
  ...
  _symbolizeColormap(e2) {
    ...
    return h(e2, this.lookup.colormapLut);
  }
  _symbolizeClassBreaks(e2) {
    ...
    return t2 ? h(e2, this.lookup.colormapLut) : d(e2, this.lookup.remapLut ?? []);
  }
  _symbolizeStretch(e2, t2) {
    ...
    if (f3 && (t2 = null), "histogramEqualization" === i.stretchType) {
      const a7 = f3 ? null : s2.histogramLut, o4 = b(i, { rasterInfo: r3, pixelBlock: e2, bandIds: t2, returnHistogramLut: !a7 }), n4 = "u8" === r3.pixelType && !f3 && a4((_a = r3.histograms) == null ? void 0 : _a[0]) ? e2 : z(e2, { ...o4, gamma: b2, isRenderer: true });
      c5 = f(n4, { lut: f3 ? o4.histogramLut : (t2 == null ? void 0 : t2.length) ? t2.map((e3) => a7[e3]) : a7, offset: 0 });
    } else if (u4) {
      let n4;
      if (f3) {
        const s3 = b(i, { rasterInfo: r3, pixelBlock: e2, bandIds: t2 });
        n4 = l2({ pixelType: a6, ...s3, gamma: b2, rounding: "floor" });
      } else {
        if (this._isLUTChanged()) {
          if (!this.bind().success) return e2;
        }
      ...
      if (!n4) return e2;
      o3 > 1 && null != t2 && t2.length === (e2 == null ? void 0 : e2.pixels.length) && n4.lut.length === o3 && (n4 = { lut: t2.map((e3) => n4.lut[e3]), offset: n4.offset }), c5 = f(e2, n4);
    } else {
      const s3 = b(i, { rasterInfo: r3, pixelBlock: e2, bandIds: t2 });
      c5 = z(e2, { ...s3, gamma: b2, isRenderer: true });
    }
    if (i.colorRamp) {
      if (this._isLUTChanged(true)) {
        if (!this.bind().success) return e2;
      }
      c5 = h(c5, s2.colormapLut);
    }
    return c5;
  }
  _symbolizeShadedRelief(e2, t2) {
    var _a;
    const r3 = this.rendererJSON, s2 = { ...r3, ...t2 }, a6 = r2(e2, s2);
    ...
    if (!o3) return a6;
    const n4 = ((_a = this.rasterInfo.statistics) == null ? void 0 : _a[0]) ?? { min: 0, max: 8e3 };
  ...
  _updateUVRenderer(e2) {
    ...
    if (null != r3) {
      ...
      r3.features.forEach((r4) => {
        var _a2, _b;
        const s3 = (_a2 = e2.uniqueValueInfos) == null ? void 0 : _a2.find((e3) => String(e3.value) === String(r4.attributes[a6])), n5 = (_b = s3 == null ? void 0 : s3.symbol) == null ? void 0 : _b.color;
      ...
      (_a = e2.uniqueValueInfos) == null ? void 0 : _a.forEach((e3) => {
        ...
        t3 ? i.push([parseInt("" + e3.value, 10)].concat(t3)) : o3 && i.push([parseInt("" + e3.value, 10)].concat(o3 == null ? void 0 : o3.color));
      });
    }
    ...
    return this.lookup = { rendererJSON: e2, colormapLut: l4 }, this.canRenderInWebGL = a3(l4 == null ? void 0 : l4.indexedColormap), { success: true };
  }
  _updateColormapRenderer(e2) {
    ...
    return this.lookup = { rendererJSON: e2, colormapLut: r3 }, this.canRenderInWebGL = a3(r3 == null ? void 0 : r3.indexedColormap), { success: true };
  }
  ...
  _analyzeClassBreaks(e2) {
    const { attributeTable: t2, pixelType: r3 } = this.rasterInfo, s2 = t2 == null ? void 0 : t2.fields.find((e3) => "value" === e3.name.toLowerCase()), a6 = t2 == null ? void 0 : t2.fields.find((t3) => t3.name.toLowerCase() === e2.field.toLowerCase()), o3 = null != s2 && null !== a6;
  ...
  _updateClassBreaksRenderer(e2) {
    ...
    if (!(n4 == null ? void 0 : n4.length)) return { success: false, error: "Unsupported renderer; missing or invalid ClassBreaksRenderer.classBreakInfos." };
    ...
    const d3 = a2({ colormap: p, fillUnspecified: false });
    return this.lookup = { rendererJSON: e2, colormapLut: d3 }, this.canRenderInWebGL = a3(d3 == null ? void 0 : d3.indexedColormap), { success: true };
  }
  _updateStretchRenderer(e2) {
    var _a, _b;
    let { stretchType: t2, dra: r3 } = e2;
    if (!("none" === t2 || ((_a = e2.statistics) == null ? void 0 : _a.length) || v(this.rasterInfo.statistics) || r3)) return { success: false, error: "Unsupported renderer; StretchRenderer.customStatistics is required when dynamic range adjustment is not used." };
    const s2 = e2.histograms || this.rasterInfo.histograms;
    !V(e2.stretchType) || (s2 == null ? void 0 : s2.length) || r3 || (t2 = "minMax");
    const { computeGamma: a6, useGamma: o3, colorRamp: n4 } = e2;
    let { gamma: i } = e2;
    if (o3 && a6 && !(i == null ? void 0 : i.length)) {
      const t3 = ((_b = e2.statistics) == null ? void 0 : _b.length) ? e2.statistics : this.rasterInfo.statistics;
    ...
    if ("histogramEqualization" === t2) {
      ...
      const t3 = b(e2, { rasterInfo: this.rasterInfo }), r4 = l2({ pixelType: l4, ...t3, gamma: o3 ? i : null, rounding: "floor" });
    ...
    return this.lookup.gamma = o3 && (i == null ? void 0 : i.length) ? i : null, this.canRenderInWebGL = "histogramEqualization" !== t2, { success: true };
  }
  ...
  _generateStretchWebGLParams(e2, t2, r3) {
    const { colormapLut: s2 } = this.lookup, a6 = t2.colorRamp ? s2 == null ? void 0 : s2.indexedColormap : null, o3 = t2.colorRamp ? s2 == null ? void 0 : s2.offset : null;
    "histogramEqualization" === t2.stretchType && (t2 = { ...t2, stretchType: "minMax" });
    const { gamma: n4 } = this.lookup, i = !(!t2.useGamma || !(n4 == null ? void 0 : n4.some((e3) => 1 !== e3)));
    let l4 = 0;
    null != e2 && (l4 = e2.getPlaneCount(), 2 === l4 && ((e2 = e2.clone()).statistics = [e2.statistics[0]], e2.pixels = [e2.pixels[0]]));
    const { bandCount: u4 } = this.rasterInfo, c5 = Math.min(3, (r3 == null ? void 0 : r3.length) || l4 || u4, u4), p = a6 || i ? 1 : 255, m2 = new Float32Array(c5);
    if (i && n4) for (let d3 = 0; d3 < c5; d3++) n4[d3] > 1 ? n4[d3] > 2 ? m2[d3] = 6.5 + (n4[d3] - 2) ** 2.5 : m2[d3] = 6.5 + 100 * (2 - n4[d3]) ** 4 : m2[d3] = 1;
    ...
    const { minCutOff: b2, maxCutOff: g, minOutput: y, maxOutput: x } = b(t2, { rasterInfo: this.rasterInfo, pixelBlock: e2, bandIds: r3 });
    1 === b2.length && (b2[2] = b2[1] = b2[0]), 1 === g.length && (g[2] = g[1] = g[0]);
    const S = g.map((e3, t3) => g[t3] === b2[t3] ? 0 : (x - y) / (g[t3] - b2[t3]) / p);
    return { bandCount: c5, minOutput: y / p, maxOutput: x / p, minCutOff: b2, maxCutOff: g, factor: S, useGamma: i, gamma: h3, gammaCorrection: f3, colormap: a6, colormapOffset: o3, stretchType: t2.stretchType, type: "stretch" };
  }
  _generateShadedReliefWebGLParams(e2, t2 = false, r3 = { x: 0, y: 0 }) {
    var _a;
    const { colormapLut: s2 } = this.lookup, a6 = e2.colorRamp ? s2 == null ? void 0 : s2.indexedColormap : null, o3 = e2.colorRamp ? s2 == null ? void 0 : s2.offset : null, n4 = { ...e2, isGCS: t2, resolution: r3 }, i = o2(n4), l4 = (_a = this.rasterInfo.statistics) == null ? void 0 : _a[0];
    return { ...i, minValue: (l4 == null ? void 0 : l4.min) ?? 0, maxValue: (l4 == null ? void 0 : l4.max) ?? 8e3, hillshadeType: "traditional" === e2.hillshadeType ? 0 : 1, type: "hillshade", colormap: a6, colormapOffset: o3 };
  }
  _generateVectorFieldWebGLParams(e2) {
    var _a, _b, _c;
    const { style: t2, inputUnit: r3, outputUnit: s2, visualVariables: a6, symbolTileSize: o3, flowRepresentation: n4 } = e2, i = ((_a = this.rasterInfo.statistics) == null ? void 0 : _a[0].min) ?? 0, l4 = ((_b = this.rasterInfo.statistics) == null ? void 0 : _b[0].max) ?? 50, u4 = (a6 == null ? void 0 : a6.find((e3) => "sizeInfo" === e3.type)) ?? { maxDataValue: l4, maxSize: 0.8 * o3, minDataValue: i, minSize: 0.2 * o3 }, c5 = u4.minDataValue ?? i, p = u4.maxDataValue ?? l4, d3 = null != u4.maxSize && null != u4.minSize ? [u4.minSize / o3, u4.maxSize / o3] : [0.2, 0.8];
    if ("wind_speed" === t2) {
      const e3 = (d3[0] + d3[1]) / 2;
      d3[0] = d3[1] = e3;
    }
    ...
    const f3 = "flow_to" === n4 === ("ocean_current_kn" === t2 || "ocean_current_m" === t2) ? 0 : Math.PI, b2 = a6 == null ? void 0 : a6.find((e3) => "rotationInfo" === e3.type);
    return { breakValues: h3, dataRange: m2, inputUnit: r3, outputUnit: s2, symbolTileSize: o3, symbolPercentRange: d3, style: t2 || "single_arrow", rotation: f3, rotationType: ((_c = this.rasterInfo.storageInfo) == null ? void 0 : _c.tileInfo) && "vector-uv" === this.rasterInfo.dataType ? "geographic" : (b2 == null ? void 0 : b2.rotationType) || e2.rotationType, type: "vectorField" };
  }
};
function V(e2) {
...
//# sourceMappingURL=chunk-2Z2MAT5Q.js.map
