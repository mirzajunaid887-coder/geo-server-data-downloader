...
function p(n, r2, c2, i2, u2, a2, f2 = null) {
  ...
  if ((m2 == null ? void 0 : m2.xmin) === c2.xmin && m2.xmax === c2.xmax && m2.ymin === c2.ymin && m2.ymax === c2.ymax) return;
  i2 = i2 ?? 0;
  ...
  for (let g2 = 0; g2 < d2.length; g2++) {
    ...
    if (null == u2 && !(u2 = j(c3, p2, n2, f2))) return;
    const { pyramidLevel: s3, pyramidResolution: m3, excessiveReading: h3 } = ce(u2, x2, a2 || "closest");
    if (h3) return;
    const { storageInfo: k } = x2, { origin: M } = k, { x: C, y: b } = m3, j2 = Math.max(0, Math.floor((r3.xmin - M.x) / C)), v = Math.max(0, Math.floor((M.y - r3.ymax) / b)), B = Math.ceil(r3.width / C - 0.1), w = Math.ceil(r3.height / b - 0.1), $ = s3 > 0 ? k.pyramidBlockWidth : k.blockWidth, I = s3 > 0 ? k.pyramidBlockHeight : k.blockHeight, E = k.blockBoundary[s3];
    if (!E) continue;
    const H = 1, P = Math.max(E.minCol, Math.floor(j2 / $) - H), W = Math.max(E.minRow, Math.floor(v / I) - H), z = Math.min(E.maxCol, Math.floor((j2 + B - 1) / $) + H), A = Math.min(E.maxRow, Math.floor((v + w - 1) / I) + H);
...
//# sourceMappingURL=chunk-JJXWMA5I.js.map
