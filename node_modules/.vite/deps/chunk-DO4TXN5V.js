...
var Heading = ({ children, class: className, hidden = false, key, level }) => {
  const DynamicHtmlTag = level ? unsafeStatic(`h${level}`) : literal`div`;
  return keyed(key, html2`<${DynamicHtmlTag} class=${safeClassMap(className)} .hidden=${hidden}>${children}</${DynamicHtmlTag}>`);
};
...
var manager = referenceElementManager({ click: true });
...
var _ActionMenu = class _ActionMenu extends LitElement {
  constructor() {
    ...
    this.mouseDownHandler = (event) => {
      ...
      this.activeMenuItemIndex = (_a = this.actionElements) == null ? void 0 : _a.findIndex((action) => action === event.target);
    };
  ...
  handleCalciteActionClick(event) {
    var _a;
    if ((_a = this.actionElements) == null ? void 0 : _a.some((action) => event.composedPath().includes(action))) {
      this.open = false;
      this.setFocus();
    }
  }
...
//# sourceMappingURL=chunk-DO4TXN5V.js.map
