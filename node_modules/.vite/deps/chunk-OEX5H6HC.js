...
function j2(e, t2, n, s, i2) {
  ...
  let A2, N2, S2, P = s / (u * q2), k = 2 * Math.PI;
  for (; Math.abs(P - k) > 1e-12; ) {
    S2 = Math.cos(2 * R + P), A2 = Math.sin(P), N2 = Math.cos(P);
    k = P, P = s / (u * q2) + x * A2 * (S2 + x / 4 * (N2 * (2 * S2 * S2 - 1) - x / 6 * S2 * (4 * A2 * A2 - 3) * (4 * S2 * S2 - 3)));
  }
  const F2 = y3 * A2 - w * N2 * g2, C = Math.atan2(y3 * N2 + w * A2 * g2, (1 - f2) * Math.sqrt(z2 + F2 * F2)), E = f2 / 16 * b2 * (4 + f2 * (4 - 3 * b2)), G = Math.atan2(A2 * m3, w * N2 - y3 * A2 * g2) - (1 - E) * f2 * v2 * (P + E * A2 * (S2 + E * N2 * (2 * S2 * S2 - 1)));
...
function q(e, t2, n, s) {
  ...
  let v2, z2, b2, j3, q2, x, A2, N2, S2, P, k = 1e3, F2 = f2;
  do {
    if (A2 = Math.sin(F2), N2 = Math.cos(F2), b2 = Math.sqrt(R * A2 * (R * A2) + (w * y3 - M * R * N2) * (w * y3 - M * R * N2)), 0 === b2) return e.distance = 0, e.azimuth = void 0, e.reverseAzimuth = void 0, e;
    q2 = M * y3 + w * R * N2, x = Math.atan2(b2, q2), S2 = w * R * A2 / b2, z2 = 1 - S2 * S2, j3 = q2 - 2 * M * y3 / z2, isNaN(j3) && (j3 = 0), P = p / 16 * z2 * (4 + p * (4 - 3 * z2)), v2 = F2, F2 = f2 + (1 - P) * p * S2 * (x + P * b2 * (j3 + P * q2 * (2 * j3 * j3 - 1)));
  } while (Math.abs(F2 - v2) > 1e-12 && --k > 0);
  if (0 === k) {
  ...
  const C = z2 * (h * h - l * l) / (l * l), E = C / 1024 * (256 + C * (C * (74 - 47 * C) - 128)), G = l * (1 + C / 16384 * (4096 + C * (C * (320 - 175 * C) - 768))) * (x - E * b2 * (j3 + E / 4 * (q2 * (2 * j3 * j3 - 1) - E / 6 * j3 * (4 * b2 * b2 - 3) * (4 * j3 * j3 - 3)))), I = Math.atan2(R * Math.sin(F2), w * y3 - M * R * Math.cos(F2)), O2 = Math.atan2(w * Math.sin(F2), w * y3 * Math.cos(F2) - M * R);
...
//# sourceMappingURL=chunk-OEX5H6HC.js.map
