...
function s() {
  return t ?? (t = (async () => {
    const s3 = await import("./basis_encoder-PLDWC2IJ.js"), t2 = await s3.default({ locateFile: (s4) => n(`esri/libs/basisu/${s4}`) });
...
function e() {
  return n2 ?? (n2 = (async () => {
    const e2 = await import("./dxt_encoder-LUFIQMOX.js");
...
async function h(e2) {
  var _a;
  const t2 = e2.data instanceof ImageBitmap ? e2.data : null;
  let n3;
  n3 = e2.data instanceof ImageBitmap ? w(e2.data) : y(e2.data, e2.width, e2.height, e2.components, e2.needsFlip);
  try {
    if (e2.hasS3TC) {
      o || await f();
      const t3 = new Uint8Array(n3.length);
      if (o == null ? void 0 : o.encode(n3, e2.width, e2.height, e2.preMultiplyAlpha, t3)) {
        const e3 = U(t3, true), n4 = [t3.buffer];
        return { result: new u((e3 == null ? void 0 : e3.internalFormat) ?? null, (e3 == null ? void 0 : e3.textureData) ?? null), transferList: n4 };
      }
      return { result: new u(null, null) };
    }
    if (e2.hasETC) {
      if (i || await d(), e2.preMultiplyAlpha && !o && await f(), e2.preMultiplyAlpha) {
        const t4 = new Uint8ClampedArray(n3.length);
        o == null ? void 0 : o.premultiply(new Uint8Array(n3), e2.width, e2.height, t4), n3 = t4;
      }
      const t3 = p(n3, e2.width, e2.height, e2.hasMipmap), r2 = t3 ? m(t3) : null, a2 = ((_a = r2 == null ? void 0 : r2.compressedTexture) == null ? void 0 : _a.levels.map((e3) => e3.buffer)) || [];
...
function p(e2, t2, n3, r2, a2 = 255, l2 = 0, s3 = false, o2 = false) {
  ...
  const h2 = u2.encode(c2), d2 = new Uint8Array(c2.buffer, 0, h2), f2 = new i.KTX2File(new Uint8Array(d2));
...
function U(e2, t2) {
  ...
  let f2 = e2.byteOffset + n3[A] + 4;
  const p2 = [];
  for (let a2 = 0; a2 < s3; ++a2) d2 = (i2 + 3 >> 2) * (o2 + 3 >> 2) * r2, h2 = new Uint8Array(e2.buffer, f2, d2), p2.push(h2), f2 += d2, i2 = Math.max(1, i2 >> 1), o2 = Math.max(1, o2 >> 1);
...
export {
  ...
  c as destroy,
  d as initializeBasisEncoder,
  f as initializeDXTEncoder
};
//# sourceMappingURL=TextureCompressionWorker-3H6V4G64.js.map
